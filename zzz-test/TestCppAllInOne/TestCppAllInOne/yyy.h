#pragma once#include <tchar.h>#include <windows.h>#include <atlbase.h>#include <atlstr.h>#include <fstream>#include <string>using std::wstring;
#include "StringOf.h"namespace SmartLib
{
	class MyFormatCode	
	{
		public:		static wstring MyFormatCodeProc(LPCTSTR strRaw)		
		{
				...
				switch (ch)				
				{
					...
					default:				
					{
						if (newLine)					
						{
							...
							{
								--strRaw;
								//go back one step to process right bracket						
							}
						}
						else					
						{
		...
		static void Case1()		
		{
			LPCWSTR strRaw = SML_WSTRING_OF(if (1) 
			{
				1 + 1;
			}
			);
