...
#ifdef _WIN32
		static int Case0(int argc, wchar_t* args[])
#else
		static int Case0(int argc, char* args[])
#endif
		{
			if (argc < 3 + 1)
			{
				printf("Usage: this.exe 1 search_term output_file\n");
				return -1;
			}
			const string_t searchTerm = args[1 + 1];
			const string_t outputFileName = args[2 + 1];
