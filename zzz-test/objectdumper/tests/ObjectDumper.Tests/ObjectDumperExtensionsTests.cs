...
namespace ObjectDumper.Tests
{
    ...
    public class ObjectDumperExtensionsTests
    {
        ...
        {
            var obj = new object();
            object output = obj.Dump("name", true);
        ...
        public void DumpToDebug_NullOrEmptyName_ThrowsArgumentNullException(string name)
        {
            ...
            Assert.Throws<ArgumentNullException>(() => obj.Dump(name, true));
        }
		...
		public void DumpToString_innerException()
		{
			...
			catch (Exception e1)
			{
				...
				catch (Exception e2)
				{
					...
					{
						var result = e1.DumpToString("test", true);
		...
		{
			var result = Node.Root.DumpToString( "root", true);
