...
INT CDiskHelper::IOCTL_DISK_GET_DRIVE_GEOMETRY_EX_Helper( IN HANDLE hDevice, OUT DISK_GEOMETRY_EX ** ppDiskGeometryEx, OUT DISK_DETECTION_INFO ** ppDiskDetectionInfo, OUT DISK_PARTITION_INFO ** ppDiskPartitionInfo )
{
	...
	do 
	{
		...
		if (!bRet)
		{
			...
			_OLD_DAPIERR((0, DRNPOS, TEXT("Allocate"), dwLastError, TEXT("[BUFFER_SIZE]=[%u]"), BUFFER_SIZE));
			nStatus = dwLastError;
			break;
		}
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_GET_DRIVE_GEOMETRY_EX"), dwLastError));
			nStatus = dwLastError;
			break;
		}
	...
	do 
	{
		...
		do 
		{
			...
			else
			{
				...
				{
					_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_GET_DRIVE_LAYOUT_EX"), dwLastError));
					nStatus = dwLastError;
					break;
				}
...
INT CDiskHelper::IOCTL_DISK_GET_PARTITION_INFO_EX_Helper( HANDLE hDevice, OUT PARTITION_INFORMATION_EX * pPartitionInformationEx )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_GET_PARTITION_INFO_EX"), dwLastError));
			nStatus = dwLastError;
			break;
		}
...
HANDLE CDiskHelper::CreateHandle( DWORD dwDiskNumber, DWORD dwDesiredAccess )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((999, DRNPOS, TEXT("CreateFile"), dwLastError, TEXT("[Disk]=[%s]"), strDisk.GetString()));
			nStatus = dwLastError;
			break;
		}
...
HANDLE CDiskHelper::CreateHandle( IN LPCTSTR szDiskNumber, IN DWORD dwDesiredAccess )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((999, DRNPOS, TEXT("CreateFile"), dwLastError, TEXT("[Disk]=[%s]"),szDiskNumber));
			nStatus = dwLastError;
			break;
		}
...
INT CDiskHelper::IOCTL_DISK_GET_LENGTH_INFO_Helper( IN HANDLE hDevice, OUT GET_LENGTH_INFORMATION * pGetLengthInformation )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_GET_LENGTH_INFO"), dwLastError));
			nStatus = dwLastError;
			break;
		}
...
INT CDiskHelper::IOCTL_STORAGE_QUERY_PROPERTY_DEVICE_Helper( IN HANDLE hDevice, OUT STORAGE_DEVICE_DESCRIPTOR ** ppStorageDeviceDescriptor )
{
	...
	do 
	{
		...
		if (!bDeviceIoControl)
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			nStatus = dwLastError;
			_OLD_DAPIERR((0, DRNPOS, TEXT("STORAGE_DEVICE_DESCRIPTOR"), dwLastError));
			break;
		}
...
INT CDiskHelper::IOCTL_STORAGE_QUERY_PROPERTY_ADAPTER_Helper( HANDLE hDevice, STORAGE_ADAPTER_DESCRIPTOR ** ppStorageAdapterDescriptor )
{
	...
	do 
	{
		...
		if (!bDeviceIoControl)
		{
			...
			nStatus = dwLastError;
			_OLD_DAPIERR((0, DRNPOS, TEXT("STORAGE_ADAPTER_DESCRIPTOR"), dwLastError));
			break;
		}
...
INT CDiskHelper::IOCTL_STORAGE_QUERY_PROPERTY_UNIQUE_ID_Helper( HANDLE hDevice, STORAGE_DEVICE_ID_DESCRIPTOR ** ppStorageDeviceIDDescriptor )
{
	...
	do 
	{
		...
		if (!bDeviceIoControl)
		{
			...
			_OLD_DSETLASTSTATUS(dwLastError);
			nStatus = dwLastError;
			_OLD_DAPIERR((0, DRNPOS, TEXT("STORAGE_DEVICE_ID_DESCRIPTOR"), dwLastError));
			break;
		}
...
INT CDiskHelper::IOCTL_STORAGE_GET_DEVICE_NUMBER_Helper( HANDLE hDevice, STORAGE_DEVICE_NUMBER * pStorageDeviceNumber )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_STORAGE_GET_DEVICE_NUMBER"), dwLastError));
			nStatus = dwLastError;
			break;
		}
...
INT CDiskHelper::IOCTL_STORAGE_GET_HOTPLUG_INFO_Helper( HANDLE hDevice, STORAGE_HOTPLUG_INFO * pStorageHotplugInfo )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_STORAGE_GET_HOTPLUG_INFO"), dwLastError));
			nStatus = dwLastError;
			break;
		}
	...
	do 
	{
		...
		do 
		{
			...
			else
			{
				...
				{
					_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_STORAGE_GET_MEDIA_TYPES_EX"), dwLastError));
					nStatus = dwLastError;
					break;
				}
...
INT CDiskHelper::IOCTL_STORAGE_QUERY_PROPERTY_ID_Helper( HANDLE hDevice, STORAGE_DEVICE_ID_DESCRIPTOR ** ppStorageDeviceIDDescriptor )
{
	...
	do 
	{
		...
		if (!bDeviceIoControl)
		{
			...
			_OLD_DSETLASTSTATUS(dwLastError);
			nStatus = dwLastError;
			_OLD_DAPIERR((999, DRNPOS, TEXT("STORAGE_DEVICE_ID_DESCRIPTOR"), dwLastError));
			break;
		}
...
INT CDiskHelper::IOCTL_DISK_CREATE_DISK_GPT_Helper( HANDLE hDevice, GUID& diskGUID )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_CREATE_DISK"), dwLastError));
			nStatus = dwLastError;
			break;
		}
...
INT CDiskHelper::IOCTL_DISK_SET_DRIVE_LAYOUT_EX_Helper( HANDLE hDevice, PDRIVE_LAYOUT_INFORMATION_EX pDriveLayoutInformationEx )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); _OLD_DSETLASTSTATUS(dwLastError);
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_SET_DRIVE_LAYOUT_EX"), dwLastError));
			nStatus = dwLastError;
			break;
		}
