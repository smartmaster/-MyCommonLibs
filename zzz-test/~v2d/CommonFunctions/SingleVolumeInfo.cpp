...
INT CSingleVolumeInfo::WriteToStorage( IStorage * pIStorage )
{
	...
	for (INT ii=0; ii<_countof(szStreamNamesOfCSingleVolumeInfo); ++ii)
	{
		...
		if (FAILED(hr))
		{
			...
			_OLD_DAPIERR((0, DRNPOS, TEXT("CreateStream"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
			nStatus = hr;
			break;
		}
		...
		if (0 == ii)
		{
			...
			{
				_OLD_DSETLASTSTATUS(hr);
				_OLD_DAPIERR((0, DRNPOS, TEXT("WriteToStream"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
				nStatus = hr;
				break;
			}
		}
		...
		else if (3 == ii)
		{
			...
			{
				_OLD_DSETLASTSTATUS(hr);
				_OLD_DAPIERR((0, DRNPOS, TEXT("spIStream->Write"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
				nStatus = hr;
				break;
			}
		}
...
INT CSingleVolumeInfo::ReadFromStorage( IStorage * pIStorage )
{
	...
	for (INT ii=0; ii<_countof(szStreamNamesOfCSingleVolumeInfo); ++ii)
	{
		...
		{
			_OLD_DSETLASTSTATUS(hr);
			_OLD_DAPIERR((0, DRNPOS, TEXT("OpenStream"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
			nStatus = hr;
			break;
		}
		...
		if (0 == ii)
		{
			...
			if (VT_BSTR != var.vt)
			{
				...
				_OLD_DAPIERR((0, DRNPOS, TEXT("VT_BSTR != var.vt"), E_INVALIDARG, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
		...
		else if (1 == ii)
		{
			...
			{
				_OLD_DSETLASTSTATUS(hr);
				_OLD_DAPIERR((0, DRNPOS, TEXT("ReadFromStream"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
				nStatus = hr;
				break;
			}
		...
		else if (3 == ii)
		{
			...
			{
				_OLD_DSETLASTSTATUS(hr);
				_OLD_DAPIERR((0, DRNPOS, TEXT("spIStream->Read"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
				nStatus = hr;
				break;
			}
		}
		...
		else if (5 == ii)
		{
			...
			{
				_OLD_DSETLASTSTATUS(hr);
				_OLD_DAPIERR((0, DRNPOS, TEXT("spIStream->Stat"), hr, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
				nStatus = hr;
				break;
			}
			...
			if (!bRet)
			{
				...
				_OLD_DAPIERR((0, DRNPOS, TEXT("Allocate"), E_OUTOFMEMORY, TEXT("[StreamNames[%d]]=[%s]"), ii, szStreamNamesOfCSingleVolumeInfo[ii]));
...
INT CSingleVolumeInfo::Retrieve( LPCTSTR szVolumeGUID, BOOL bGetVolumeBitmap, BOOL bBreakOnError)
{
	...
	do 
	{
		...
		if (nStatus)
		{
			_OLD_DAPIERR((999, DRNPOS, TEXT("GetVolumePathFromGuid"), nStatus));
			//nStatus = E_FAIL;
			if (bBreakOnError)
			{
				break;
		...
		if (nStatus)
		{
			_OLD_DAPIERR((999, DRNPOS, TEXT("GetVolumeMountPoint"), nStatus));
		...
		if (nStatus)
		{
			_OLD_DAPIERR((999, DRNPOS, TEXT("GetVolumeInfo"), nStatus));
			//nStatus = E_FAIL;
			if (bBreakOnError)
			{
				break;
		...
		if (nStatus)
		{
			_OLD_DAPIERR((0, DRNPOS, TEXT("FSCTL_GET_NTFS_VOLUME_DATA_Helper"), E_FAIL));
		...
		if (nStatus)
		{
			_OLD_DAPIERR((999, DRNPOS, TEXT("IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS_Helper"), nStatus));
			//nStatus = hr;
			if (bBreakOnError)
			{
				break;
		...
		if (bGetVolumeBitmap) //******************************************
		{
			...
			if (nStatus)
			{
				_OLD_DAPIERR((0, DRNPOS, TEXT("FSCTL_GET_VOLUME_BITMAP_Helper"), E_FAIL));
				//nStatus = hr;
				if (bBreakOnError)
				{
					break;
		...
		if (nStatus)
		{
			_OLD_DAPIERR((0, DRNPOS, TEXT("IOCTL_DISK_GET_LENGTH_INFO_Helper"), E_FAIL));
			//nStatus = hr;
			if (bBreakOnError)
			{
				break;
...
INT CAllVolumeInfo::Retrieve( BOOL bGetVolumeBitmap )
{
	...
	for (size_t ii=0; ii<VolumeGuids.size(); ++ii)
	{
		...
		if (nStatus)
		{
			_OLD_DAPIERR((999, DRNPOS, TEXT("Retrieve"), nStatus, TEXT("[VolumeGuid=%s]"), VolumeGuid.GetString()));
		}
		else 
		{
