...
void VssClient::InitializeWriterMetadata()
{
	...
	// Enumerate writers
	m_writerList.resize(cWriters); //<mc> 2010-7-17 10:56
	for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
	{
		// Get the metadata for this particular writer
		VSS_ID idInstance = GUID_NULL;
		...
		VssWriter  & writer = m_writerList[iWriter]; //<mc> 2010-7-17 10:56
		writer.Initialize(pMetadata);

		// Add this writer to the list 
		//m_writerList.push_back(writer); //<mc> 2010-7-17 10:55
	}
}
...
void VssClient::InitializeWriterComponentsForRestore()
{
	...
	// Enumerate writers
	m_writerComponentsForRestore.reserve(cWriters); //<mc> 2010-7-17 10:58
	for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
	{
		...
		for (unsigned i = 0; i < m_writerList.size(); i++)
		{
			...
			m_writerComponentsForRestore.push_back(m_writerList[i]); //<mc> 2010-7-17 10:59
			VssWriter &  writer = m_writerComponentsForRestore[m_writerComponentsForRestore.size() - 1]; //<mc> 2010-7-17 11:00
			...
			//m_writerComponentsForRestore.push_back(writer); //<mc> 2010-7-17 10:58
...
void VssWriter::Initialize(IVssExamineWriterMetadata * pMetadata)
{
	...
	// Get exclude files
	m_excludedFiles.resize(cExcludeFiles); //<mc> 2010-7-17 11:01
	for(unsigned i = 0; i < cExcludeFiles; i++)
	{
		...
		//excludedFiles.push_back(excludedFile); //<mc> 2010-7-17 11:00
	}

	// Enumerate components
	m_components.resize(cComponents); //<mc> 2010-7-17 11:02
	for(unsigned iComponent = 0; iComponent < cComponents; iComponent++)
	{
		...
		//components.push_back(component); //<mc> 2010-7-17 11:02
	}
...
void VssWriter::InitializeComponentsForRestore(IVssWriterComponentsExt * pWriterComponents)
{
	...
	// Enumerate components
	m_components.resize(cComponents); //<mc> 2010-7-17 11:04
	for(unsigned iComponent = 0; iComponent < cComponents; iComponent++)
	{
		// Get component
		...
		// Add this component to the list of components
		VssComponent & component = m_components[iComponent]; //<mc> 2010-7-17 11:04
		component.Initialize(m_name, pComponent);

		ft.WriteLine(L"- Found component available for restore: \"%s\"", component.m_fullPath.c_str());

		//components.push_back(component); //<mc> 2010-7-17 11:03
	}
}
...
void VssComponent::Initialize(wstring writerNameParam, IVssWMComponent * pComponent)
{
	...
	m_descriptors.resize(pInfo->cFileCount + pInfo->cDatabases + pInfo->cLogFiles); //<mc> 2010-7-17 11:04
	INT nIndexFile = 0;
	for(unsigned i = 0; i < pInfo->cFileCount; i++)
	{
		...
		//descriptors.push_back(desc); //<mc> 2010-7-17 11:05
	}
	...
	for(unsigned i = 0; i < pInfo->cDependencies; i++)
	{
		...
		//dependencies.push_back(dependency); //<mc> 2010-7-17 11:06
	}
#endif
	...
	)
{
	...
	m_expandedPath.resize(MAX_PATH, L'\0');
	_ASSERTE(bstrPath && bstrPath[0]);
	CHECK_WIN32(ExpandEnvironmentStringsW(bstrPath, (PWCHAR)m_expandedPath.c_str(), (DWORD)m_expandedPath.length()));
	m_expandedPath = AppendBackslash(m_expandedPath);
