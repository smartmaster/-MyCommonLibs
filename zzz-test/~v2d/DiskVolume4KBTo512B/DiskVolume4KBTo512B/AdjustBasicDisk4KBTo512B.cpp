...
static BOOL Is4KBSectorExtendedPartition(IFileDevice * pDdisk, LONGLONG OffsetLBA)
{
	...
	if (FAILED(hr))
	{
		DAPIERR(0, TEXT("ReadAtOffsetHelper"), hr, TEXT("Offset=%I64d, ToRead=%d"), __512BOffset, BUFFER_SIZE);
		LEAVE_BLOCK(0);
	}

	if (IsValidBootRecord(apBuffer.m_pData))
	{
		DINFO(0, TEXT("Found a boot EBR at %I64d * 512B, assume it is a 512B sector disk"), OffsetLBA);
	...
	if (FAILED(hr))
	{
		DAPIERR(0, TEXT("ReadAtOffsetHelper"), hr, TEXT("Offset=%I64d, ToRead=%d"), __4KBOffset, BUFFER_SIZE);
		LEAVE_BLOCK(0);
	}
	...
	if (IsValidBootRecord(apBuffer.m_pData))
	{
		...
		DINFO(0, TEXT("Found a EBR at %I64d * 4KB, assume it is a 4KB sector disk"), OffsetLBA);
		LEAVE_BLOCK(0);
	}
...
static BOOL Is4KBSectorDataPartition(IFileDevice * pDdisk, LONGLONG OffsetLBA)
{
	...
	{
		DINFO(0, TEXT("Found a boot sector at %I64d * 512B, assume it is a 512B sector disk"), OffsetLBA);
		LEAVE_BLOCK(0); // 512B sector disk, not 4KB sector disk
	}
	...
	{
		bRet = TRUE; // 4KB sector disk
		DINFO(0, TEXT("Found a boot sector at %I64d * 4KB, assume it is a 4KB sector disk"), OffsetLBA);
		LEAVE_BLOCK(0); 
	}
...
static BOOL Is4KBSectorBasicMBRDisk(IFileDevice * pDiskRW)
{
	...
	DINFO(0, TEXT("Found %d partitions of 4KB sector"), __4KBPartCount);
	if (0 == __4KBPartCount)
	{
		return FALSE;
	}
...
static HRESULT Adjust4KBVolume(IFileDevice * pDiskRW, LONGLONG VolumeOffset/*, BYTE PartID*/)
{
	...
	if (NULL == apVolumeBootSector4KB.m_pData)
	{
		hr = E_OUTOFMEMORY;
		DSETLASTSTATUS(hr, -1);
		DAPIERR(0, TEXT("Allocate"), hr, TEXT(""));
		LEAVE_BLOCK(0);
	}
	...
	{
		bAdjusted = TRUE;
		//DINFO(0, TEXT("the volume at offset %I64d is a NTFS volume, adjust 4KB sector to 512B [PartitionID=%d, Description=%s]"), VolumeOffset, PartID, MBRPartitionTypeString(PartID));
		DINFO(0, TEXT("the volume at offset %I64d is a NTFS volume, adjust 4KB sector to 512B"), VolumeOffset);
	...
	{
		//DINFO(0, TEXT("the volume at offset %I64d is a ReFS volume, no need to adjust 4KB sector to 512B [PartitionID=%d, Description=%s]"), VolumeOffset, PartID, MBRPartitionTypeString(PartID));
		DINFO(0, TEXT("the volume at offset %I64d is a ReFS volume, no need to adjust 4KB sector to 512B"), VolumeOffset);
	} 
	else if (IsFATSignature(pBootSector->oem))
	{
		...
		{
			bAdjusted = TRUE;
			//DINFO(0, TEXT("the volume at offset %I64d is a FAT32 volume, adjust 4KB sector to 512B [PartitionID=%d, Description=%s]"), VolumeOffset, PartID, MBRPartitionTypeString(PartID));
			DINFO(0, TEXT("the volume at offset %I64d is a FAT32 volume, adjust 4KB sector to 512B"), VolumeOffset);
			AdjustFAT4KBTo512B(apVolumeBootSector4KB.m_pData, apVolumeBootSector512B.m_pData, BUFFER_SIZE);
		}
		else
		{
			//DINFO(0, TEXT("the volume at offset %I64d is a FAT12/16 volume, not support to adjust 4KB sector to 512B [PartitionID=%d, Description=%s]"), VolumeOffset, PartID, MBRPartitionTypeString(PartID));
			DINFO(0, TEXT("the volume at offset %I64d is a FAT12/16 volume, not support to adjust 4KB sector to 512B"), VolumeOffset);
		}
	}
	else
	{
		//DINFO(0, TEXT("the volume at offset %I64d cannot be recognized, not support to adjust 4KB sector to 512B [PartitionID=%d, Description=%s]"), VolumeOffset, PartID, MBRPartitionTypeString(PartID));
		DINFO(0, TEXT("the volume at offset %I64d cannot be recognized, not support to adjust 4KB sector to 512B"), VolumeOffset);
	}
	...
	if (bAdjusted)
	{
		...
		if (FAILED(hr))
		{
			DAPIERR(0, TEXT("WriteAtOffsetHelper"), hr, TEXT(""));
			LEAVE_BLOCK(0);
		}
	}
...
HRESULT AdjustBasicGPT4KBDisk(IFileDevice * pDiskRW)
{
	...
	//{
	//	DINFO(0, TEXT("Not a 4KBSectorBasicGPTDisk, skipping adjusting"));
	//	LEAVE_BLOCK(0);
	//}
	...
	for (INT ii = 0; ii < 128; ++ ii)
	{
		...
		DINFO(0, TEXT("GPT partition number %d is at offset %I64d"), ii, VolumeOffset);
		HRESULT hrVolume = Adjust4KBVolume(pDiskRW, VolumeOffset/*, 0xee*/);
		if (FAILED(hrVolume))
		{
			DSETLASTSTATUS(hrVolume, -1);
			DAPIERR(0, TEXT("Adjust4KBVolume"), hrVolume, TEXT("partition number is %d at offset %I64d"), ii, VolumeOffset);
			//LEAVE_BLOCK(xxxx);
			hr = hrVolume;
		}
	}
...
HRESULT AdjustBasicMBR4KBDisk(IFileDevice * pDiskRW, LONGLONG CurrentMBROffset, LONGLONG & FirstEBROffset)
{
	...
	//{
	//	DINFO(0, TEXT("Not a 4KBSectorBasicMBRDisk, skipping adjusting"));
	//	LEAVE_BLOCK(0);
	//}
		...
		if (pPTE[ii].relativeSectors && pPTE[ii].totalSectors)
		{
			...
			if (IsExtendedPartition(pPTE[ii].SystemID))
			{
				if (0 == FirstEBROffset)
				{
					...
					DINFO(0, TEXT("Found the first extended partition at offset %I64d (0x%I64x), partition number %d"), SubMbrOffset, SubMbrOffset, ii);
				}
				else
				{
					...
					DINFO(0, TEXT("Found extended partition at offset %I64d (0x%I64x), partition number %d"), SubMbrOffset, SubMbrOffset, ii);
				}
			...
			else
			{
				...
				DINFO(0, TEXT("Found partition at offset %I64d [PartitionID=%d, Description=%s]"), SubMbrOffset, pPTE[ii].SystemID, MBRPartitionTypeString(pPTE[ii].SystemID));
...
HRESULT Adjust4KBSectorBasicDisk( IFileDevice * pDiskRW )
{
	...
	{
		DINFO(0, TEXT("Adjust 4KB sector basic MBR disk"));
		LONGLONG FirstEBROffset = 0;
		hr = AdjustBasicMBR4KBDisk(pDiskRW, 0, FirstEBROffset);
	}
	else if (Is4KBSectorBasicGPTDisk(pDiskRW))
	{
		DINFO(0, TEXT("Adjust 4KB sector basic GPT disk"));
		hr = AdjustBasicGPT4KBDisk(pDiskRW);
	}
	else
	{
		DINFO(0, TEXT("it is neither a 4KB-sector basic GPT disk nor a 4KB-sector basic MBR disk, skip adjusting" ));
	}
	return hr;
}
