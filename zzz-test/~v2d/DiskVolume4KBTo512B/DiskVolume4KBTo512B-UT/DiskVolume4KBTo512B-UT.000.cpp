...
int tmain_adjust_basic_disk(int argc, _TCHAR* argv[])
{
    for (ULONG diskNumber = 0; diskNumber < 1023; ++ diskNumber)
    {
        HRESULT hr = S_OK;

        BEGIN_BLOCK(0);
        CString strStorage;
        strStorage.Format(TEXT("\\\\.\\PhysicalDrive%u"), diskNumber);
        //////////////////////////////////////////////////////////////////////////
        CComPtr<IFileDevice> spStorage = NULL;
        hr = CreateInstanceFileDeviceWin32(
            strStorage.GetString(),//IN LPCTSTR strFileName, 
...
{
	DSETTINGS(99, FALSE);
    return tmain_adjust_basic_disk(argc, argv);
