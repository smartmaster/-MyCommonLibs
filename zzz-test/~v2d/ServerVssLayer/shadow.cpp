...
//
INT VShadowMainEntry(__in int argc, __in_ecount(argc) WCHAR ** argv)
{
    ...
    try
    {
        ...
        return obj.MainRoutine(arguments, NULL, NULL, NULL);
    }
    catch(bad_alloc ex)
    {
...
HRESULT VShadowComMainEntry(SAFEARRAY * psaArgvs, IStorage * pISorageXmls, IStorage * pISorageVssResults, DWORD * pdwCookieIVssBackupComponents/*IVssBackupComponents ** ppVssBackupComponents*/)
{
	...
	try
	{
		...
		CHECK_COM(spsa.Attach(psaArgvs));

		INT argc = spsa.GetCount();
		vector<wstring> arguments;
		for(int i = 0; i < argc; i++)
		{
			arguments.push_back(LPCTSTR(spsa[i]));
		}

		spsa.Detach();

		// Process the arguments and execute the main options
		return (0 == obj.MainRoutine(arguments, pISorageXmls, pISorageVssResults, pdwCookieIVssBackupComponents)? S_OK: E_FAIL);
	}
	catch(bad_alloc ex)
	{
		// Generic STL allocation error
...
//
int CommandLineParser::MainRoutine(
	CONST vector<wstring> & arguments,
	IStorage * pIStorageXmls, 
	IStorage * pIStorageVssResults,
	DWORD * pdwCookieIVssBackupComponents
	) //<mc> 2010-7-25 10:54
{
    FunctionTracer ft(DBG_INFO);
    ...
    for(unsigned argIndex = 0; argIndex < arguments.size(); argIndex++)
    {
        ...
        if (IsVolume(arguments[argIndex]))
        {
			...
			//******************************************
			m_vssClient.InitializeStorage(pIStorageXmls, pIStorageVssResults, pdwCookieIVssBackupComponents); //<mc> 2010-8-21 19:10);
			...
			if (pdwCookieIVssBackupComponents)
			{
				CComGITPtr<IVssBackupComponents> spGITPtr(m_vssClient.RawVssObjectPtr());
				*pdwCookieIVssBackupComponents = spGITPtr.Detach();
			}
...
bool CommandLineParser::MatchArgument(CONST wstring & argument, CONST wstring & optionPattern, wstring & additionalParameter)
{
    FunctionTracer ft(DBG_INFO);
