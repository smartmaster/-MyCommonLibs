...
VssClient::VssClient()
{
	...
	m_pIStorageXmls = NULL; //outside object
	m_pIStorageVssResults = NULL; //outside object
	m_pdwCookieIVssBackupComponents = NULL;
}
...
void VssClient::Initialize(DWORD dwContext, wstring xmlDoc, bool bDuringRestore)
{
	...
	CHECK_COM_ERROR( hr, TEXT("Com Initialize")) ;
...
VOID VssClient::InitializeStorage( IStorage * pIStorageXmls, IStorage * pIStorageVssResults, DWORD * pdwCookieIVssBackupComponents )
{
	m_pIStorageXmls = pIStorageXmls;
	m_pIStorageVssResults = pIStorageVssResults;
	m_pdwCookieIVssBackupComponents = pdwCookieIVssBackupComponents;
}
