...
void VssClient::QuerySnapshotSet(VSS_ID snapshotSetID)
{
    ...
    while(true)
    {
		...
		if (m_pIStorageVssResults)
		{
			CString strStorageName;
			strStorageName.Format(STG_ITEM_NAME, nIndex);
			CComPtr<IStorage> spIStorage;
			hr = m_pIStorageVssResults->CreateStorage(
			...
			CHECK_COM_ERROR(hr, TEXT("CreateStorage"));

			//******************************************
			MY_VSS_SNAPSHOT_PROP MyVssSnapshotProp = {0};
			MyVssSnapshotProp.Copy(Snap);
			INT nStatusWriteToStorage = MyVssSnapshotProp.WriteToStorage(spIStorage);
			if (nStatusWriteToStorage)
			{
				CHECK_COM_ERROR(E_FAIL, TEXT("WriteToStorage"));
			}
		}
    }
	...
	if (m_pIStorageVssResults)
	{
		...
		hr = m_pIStorageVssResults->CreateStream(
		...
		CHECK_COM_ERROR(hr, TEXT("CreateStream"));

		//******************************************
		CComVariant var = (ULONG)(nIndex + 1);
		hr = var.WriteToStream(spIStream);
		CHECK_COM_ERROR(hr, TEXT("WriteToStream"));
	}
}
