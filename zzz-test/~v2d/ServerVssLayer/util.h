...
inline string WString2String(CONST wstring & src)
{
...
inline GUID & WString2Guid(CONST wstring & src)
{
...
inline vector<wstring> SplitWString(CONST wstring & str, WCHAR separator)
{
    FunctionTracer ft(DBG_INFO);
...
inline wstring Guid2WString(CONST GUID & guid)
{
    FunctionTracer ft(DBG_INFO);
...
inline bool FindStringInList(CONST wstring & str, CONST vector<wstring> & stringList)
{
...
inline const wstring AppendBackslash(CONST wstring & str)
{
    if (str.length() == 0)
        return wstring(L"\\");
...
inline bool IsVolume(CONST wstring & volPath)
{
    FunctionTracer ft(DBG_INFO);

    bool bIsVolume = false;
    ft.Trace(DBG_INFO, L"Checking if %s is a real volume path...", volPath.c_str());
    _ASSERTE(volPath.length() > 0);
    
    // If the last character is not '\\', append one
    wstring volumePath = AppendBackslash(volPath);
...
inline wstring GetUniqueVolumeNameForMountPoint(CONST wstring & mntPoint)
{
    FunctionTracer ft(DBG_INFO);

    _ASSERTE(mntPoint.length() > 0);

    ft.Trace(DBG_INFO, L"- Get volume name for %s ...", mntPoint.c_str());

    // Add the backslash termination, if needed
    wstring mountPoint = AppendBackslash(mntPoint);
...
inline wstring GetUniqueVolumeNameForPath(CONST wstring & path0, bool bIsBackup=false)
{
    FunctionTracer ft(DBG_INFO);

    _ASSERTE(path0.length() > 0);

    wstring volumeRootPath(MAX_PATH, L'\0');
    wstring volumeUniqueName(MAX_PATH, L'\0');

    ft.Trace(DBG_INFO, L"- Get volume path name for %s ...", path0.c_str());

    // Add the backslash termination, if needed
    wstring path = AppendBackslash(path0);
...
inline bool GetUniqueVolumeNameForPathNoThrow(CONST wstring & path0, wstring &volname)
{
    FunctionTracer ft(DBG_INFO);
...
inline wstring GetDeviceForVolumeName(wstring volumeName) //<mc> 2010-7-17 11:27 no need ro change
{
    FunctionTracer ft(DBG_INFO);
...
inline wstring GetDisplayNameForVolume(CONST wstring & volumeName)
{
    FunctionTracer ft(DBG_INFO);
...
inline bool GetDisplayNameForVolumeNoThrow(CONST wstring & volumeName, wstring &volumeNameCanon)
{
    FunctionTracer ft(DBG_INFO);
...
inline wstring & ReadFileContents(wstring fileName, wstring & contents)
{
    FunctionTracer ft(DBG_INFO);
    ...
    contents.resize(dwFileSize / sizeof(WCHAR), L'\0');
...
inline void WriteFile(CONST wstring & fileName, CONST wstring & contents)
{
    FunctionTracer ft(DBG_INFO);
...
// Execute a command
inline void ExecCommand(wstring execCommand) //<mc> 2010-7-17 11:38 no need to change
{
    FunctionTracer ft(DBG_INFO);
