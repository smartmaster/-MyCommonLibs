...
void VssClient::GenerateSetvarScript(wstring stringFileName)
{
    ...
    ofile << L"@echo.  --*r*n*--  ";
    ofile << L"@echo [This script is generated by VSHADOW.EXE for the shadow set " << Guid2WString(m_latestSnapshotSetID).c_str() << L"]  --*r*n*--  ";
    ofile << L"@echo.  --*r*n*--    --*r*n*--  ";

    wstring snapshotSetID = Guid2WString(m_latestSnapshotSetID);
    ofile << L"SET SHADOW_SET_ID=" << snapshotSetID.c_str() << L"  --*r*n*--  ";
    ...
    for (unsigned i = 0; i < m_latestSnapshotIdList.size(); i++)
    {
        ...
        ofile << L"SET SHADOW_ID_" << i+1 << L"=" << snapshotID.c_str() << L"  --*r*n*--  ";
        ...
        if ((m_dwContext & VSS_VOLSNAP_ATTR_TRANSPORTABLE) == 0)
        {
            ...
            ofile << L"SET SHADOW_DEVICE_" << i+1 << L"=" << Snap.m_pwszSnapshotDeviceObject << L"  --*r*n*--  ";
        }
    }
