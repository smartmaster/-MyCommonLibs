...
// Tracing routine
// Can throw HRESULT on invalid formats
void FunctionTracer::Trace(CONST wstring & file, int line, CONST wstring & functionName, wstring format, ...)
{
	if (m_traceEnabled)
	{
		wstring buffer;
		...
		//D_INFO(0, L"[[%40s @ %10s:%4d]] %s  --*r*n*--  ", functionName.c_str(), fileName.c_str(), line, buffer.c_str());
		D_INFO(0, L"%s  --*r*n*--  ", buffer.c_str());
	}
}
