...
void VssClient::ExposeSnapshotLocally(VSS_ID snapshotID, wstring path)
{
    ...
    {
        ft.WriteLine(L"  --*r*n*--  ERROR: there is no snapshot with the given ID");
        throw(E_INVALIDARG);
    }
    ...
    {
        ft.WriteLine(L"  --*r*n*--  ERROR: the snapshot ID identifies a Client Accessible snapshot which cannot be exposed");
        throw(E_INVALIDARG);
    }
    ...
    {
        ft.WriteLine(L"  --*r*n*--  ERROR: Client-accessible (SFSF) snapshots cannot be exposed.");
        throw(E_INVALIDARG);
    }
    ...
    if ((path.length() == 2) && path[1] == L':')
    {
        ...
        {
            ft.WriteLine(L"  --*r*n*--  ERROR: the second parameter to -el [%s] is a drive letter already in use!", path.c_str());
            throw(E_INVALIDARG);
        }
    }
    else
    {
        ...
        {
            ft.WriteLine(L"  --*r*n*--  ERROR: the second parameter to -el [%s] is not a valid directory!", path.c_str());
            throw(E_INVALIDARG);
        }
        ...
        { 
            ...
            {
                ft.WriteLine(L"  --*r*n*--  ERROR: the second parameter to -el [%s] is not an empty directory!", path.c_str());
                throw(E_INVALIDARG);
            }
