...
bool OSVersionCheck()
{
    ...
    if ( ::GetVersionExW( (LPOSVERSIONINFO)&osv ) )
    {
        // Must be at least Windows 8
...
int VShadowMain(__in int argc, __in_ecount(argc) WCHAR ** argv)
{
    ...
    try
    {
        ...
        ft.WriteLine(
            L"  --*r*n*--  "
            L"VSHADOW.EXE 3.0 - Volume Shadow Copy sample client.  --*r*n*--  "
            L"Copyright (C) 2005 Microsoft Corporation. All rights reserved.  --*r*n*--  "
            L"  --*r*n*--  "
            );
    ...
    for(unsigned argIndex = 0; argIndex < arguments.size(); argIndex++)
    {
        ...
        if (MatchArgument(arguments[argIndex], L"bw", id))
        {
            ...
            if (m_bWaitForFinish || execCommand.length()>0)
            {
                ...
                if (m_bWaitForFinish)
                {
                    ft.WriteLine(L"  --*r*n*--  Press <ENTER> to make the volumes writable...");
#pragma warning(suppress: 6031)  //Intentionally ignore the return value of getchar()
        ...
        if (MatchArgument(arguments[argIndex], L"i", xmlBackupComponentsDoc))
        {
            ...
            wstring xmlDoc;
			ReadFileContents(xmlBackupComponentsDoc, xmlDoc);
        ...
        if (MatchArgument(arguments[argIndex], L"r", xmlBackupComponentsDoc))
        {
            ...
            ft.WriteLine(L"  --*r*n*--  Restore done.");
        ...
        if (MatchArgument(arguments[argIndex], L"rs", xmlBackupComponentsDoc))
        {
            ...
            ft.WriteLine(L"  --*r*n*--  Restore simulation done.");
        ...
        if (IsVolume(arguments[argIndex]) || IsUNCPath((VSS_PWSZ)arguments[argIndex].c_str()))
        {
            ...
            ft.Trace(DBG_INFO, L"  --*r*n*--  Attempting to create a shadow copy set... (volume %s was added as parameter)", arguments[argIndex].c_str());
            ...
            for(unsigned i = argIndex + 1; i < arguments.size(); i++)
            {
                if (!(IsVolume(arguments[i]) || IsUNCPath((VSS_PWSZ)arguments[i].c_str())))
                {
                    ...
                    ft.WriteLine(L"  --*r*n*--  ERROR: invalid parameters %s", GetCommandLine());
            ...
            ft.WriteLine(L"  --*r*n*--  Snapshot creation done.");
...
bool CommandLineParser::MatchArgument(wstring argument, wstring optionPattern)
{
    ...
    ft.Trace(DBG_INFO, L"Matching Arg: '%s' with '%s'  --*r*n*--  ", argument.c_str(), optionPattern.c_str());

    bool retVal = (IsEqual(argument, wstring(L"/") + optionPattern) || IsEqual(argument, wstring(L"-") + optionPattern) );

    ft.Trace(DBG_INFO, L"Return: %s  --*r*n*--  ", BOOL2TXT(retVal));
    return retVal;
}
...
void CommandLineParser::PrintUsage()
{
    ...
    ft.WriteLine(
        L"Usage:  --*r*n*--  "
        L"   VSHADOW [optional flags] [commands]  --*r*n*--  "
        L"  --*r*n*--  "
        L"List of optional flags:  --*r*n*--  "
        L"  -?                 - Displays the usage screen  --*r*n*--  "
        L"  -p                 - Manages persistent shadow copies  --*r*n*--  "
        L"  -nw                - Manages no-writer shadow copies  --*r*n*--  "
        L"  -nar               - Creates shadow copies with no auto-recovery  --*r*n*--  "
        L"  -tr                - Creates TxF-recovered shadow copies  --*r*n*--  "
        L"  -ad                - Creates differential HW shadow copies  --*r*n*--  "
        L"  -ap                - Creates plex HW shadow copies  --*r*n*--  "
        L"  -scsf              - Creates Shadow Copies for Shared Folders (Client Accessible)  --*r*n*--  "
        L"  -t={file.xml}      - Transportable shadow set. Generates also the backup components doc.  --*r*n*--  "
        L"  -bc={file.xml}     - Generates the backup components doc for non-transportable shadow set.  --*r*n*--  "
        L"  -wi={Writer Name}  - Verify that a writer/component is included  --*r*n*--  "
        L"  -wx={Writer Name}  - Exclude a writer/component from set creation or restore  --*r*n*--  "
        L"  -mask              - BreakSnapshotSetEx flag: Mask shadow copy luns from system on break.  --*r*n*--  "
        L"  -rw                - BreakSnapshotSetEx flag: Make shadow copy luns read-write on break.  --*r*n*--  "
        L"  -forcerevert       - BreakSnapshotSetEx flag: Complete operation only if all disk signatures revertable.  --*r*n*--  "
        L"  -norevert          - BreakSnapshotSetEx flag: Do not revert disk signatures.  --*r*n*--  "
        L"  -revertsig         - Revert to the original disk's signature during resync.  --*r*n*--  "
        L"  -novolcheck        - Ignore volume check during resync. Unselected volumes will be overwritten.  --*r*n*--  "
        L"  -script={file.cmd} - SETVAR script creation  --*r*n*--  "
        L"  -exec={command}    - Custom command executed after shadow creation, import or between break and make-it-write  --*r*n*--  "
        L"  -wait              - Wait before program termination or between shadow set break and make-it-write  --*r*n*--  "
        L"  -tracing           - Runs VSHADOW.EXE with enhanced diagnostics  --*r*n*--  "
        L"  --*r*n*--  " );
    ft.WriteLine(
        L"List of commands:  --*r*n*--  "
        L"  {volume list}                   - Creates a shadow set on these volumes  --*r*n*--  "
        L"  -ws                             - List writer status  --*r*n*--  "
        L"  -wm                             - List writer summary metadata  --*r*n*--  "
        L"  -wm2                            - List writer detailed metadata  --*r*n*--  "
        L"  -wm3                            - List writer detailed metadata in raw XML format  --*r*n*--  "
        L"  -q                              - List all shadow copies in the system  --*r*n*--  "
        L"  -qx={SnapSetID}                 - List all shadow copies in this set  --*r*n*--  "
        L"  -s={SnapID}                     - List the shadow copy with the given ID  --*r*n*--  "
        L"  -da                             - Deletes all shadow copies in the system  --*r*n*--  "
        L"  -do={volume}                    - Deletes the oldest shadow of the specified volume  --*r*n*--  "
        L"  -dx={SnapSetID}                 - Deletes all shadow copies in this set  --*r*n*--  "
        L"  -ds={SnapID}                    - Deletes this shadow copy  --*r*n*--  "
        L"  -i={file.xml}                   - Transportable shadow copy import  --*r*n*--  "
        L"  -b={SnapSetID}                  - Break the given shadow set into read-only volumes  --*r*n*--  "
        L"  -bw={SnapSetID}                 - Break the shadow set into writable volumes  --*r*n*--  "
        L"  -bex={SnapSetID}                - Break using BreakSnapshotSetEx and flags, see options for available flags  --*r*n*--  "
        L"  -el={SnapID},dir                - Expose the shadow copy as a mount point  --*r*n*--  "
        L"  -el={SnapID},drive              - Expose the shadow copy as a drive letter  --*r*n*--  "
        L"  -er={SnapID},share              - Expose the shadow copy as a network share  --*r*n*--  "
        L"  -er={SnapID},share,path         - Expose a child directory from the shadow copy as a share  --*r*n*--  "
        L"  -r={file.xml}                   - Restore based on a previously-generated Backup Components document  --*r*n*--  "
        L"  -rs={file.xml}                  - Simulated restore based on a previously-generated Backup Components doc  --*r*n*--  "\
        L"  -revert={SnapID}                - Revert a volume to the specified shadow copy  --*r*n*--  "
        L"  -addresync={SnapID},drive       - Resync the given shadow copy to the specified volume  --*r*n*--  "
        L"  -addresync={SnapID}             - Resync the given shadow copy to it's original volume  --*r*n*--  "
        L"  -resync=bcd.xml                 - Perform Resync using the specified BCD  --*r*n*--  "
        L"  --*r*n*--  " );
    ft.WriteLine(
        L"Examples:  --*r*n*--  "
        L"  --*r*n*--  "
        L" - Non-persistent shadow copy creation on C: and E:  --*r*n*--  "
        L"     VSHADOW C: E:  --*r*n*--  "
        L"  --*r*n*--  "
        L" - Non-persistent shadow copy creation on a CSV named Volume1  --*r*n*--  "
        L"     VSHADOW C:\\ClusterStorage\\Volume1  --*r*n*--  "
        L"  --*r*n*--  "
        L" - Persistent shadow copy creation on C: (with no writers)  --*r*n*--  "
        L"     VSHADOW -p -nw C:  --*r*n*--  "
        L"  --*r*n*--  "
        L" - Transportable shadow copy creation on X:  --*r*n*--  "
        L"     VSHADOW -t=file1.xml X:  --*r*n*--  "
        L"  --*r*n*--  "
        L" - Transportable shadow copy import  --*r*n*--  "
        L"     VSHADOW -i=file1.xml  --*r*n*--  "
        L"  --*r*n*--  "
        L" - List all shadow copies in the system:  --*r*n*--  "
        L"     VSHADOW -q  --*r*n*--  "
        L"  --*r*n*--  "
        L"Please see the README.DOC file for more details.  --*r*n*--  "
        L"  --*r*n*--  "
        L"  --*r*n*--  "
        );
#else
    ft.WriteLine(
        ...
        L"  -wi={Writer Name}  - Verify that a writer/component is included  --*r*n*--  "
        L"  -wx={Writer Name}  - Exclude a writer/component from set creation or restore  --*r*n*--  "
        L"  -bc={file.xml}     - Generates the backup components document during shadow creation.  --*r*n*--  "
        L"  -script={file.cmd} - SETVAR script creation  --*r*n*--  "
        L"  -exec={command}    - Custom command executed after shadow creation  --*r*n*--  "
        L"  -wait              - Wait before program termination   --*r*n*--  "
        L"  -tracing           - Runs VSHADOW.EXE with enhanced diagnostics  --*r*n*--  "
        L"  --*r*n*--  "
        L"List of commands:  --*r*n*--  "
        L"  {volume list}      - Creates a shadow set on these volumes  --*r*n*--  "
        ...
        L"  -dx={SnapSetID}    - Deletes all shadow copies in this set  --*r*n*--  "
        L"  -ds={SnapID}       - Deletes this shadow copy  --*r*n*--  "
        L"  -r={file.xml}      - Restore based on a previously-generated Backup Components doc  --*r*n*--  "
        L"  -rs={file.xml}     - Simulated restore based on a previously-generated Backup Components doc  --*r*n*--  "
        L"  --*r*n*--  ");
    ft.WriteLine(
        L"Examples:  --*r*n*--  "
        L"  --*r*n*--  "
        L" - Non-persistent shadow copy creation on C: and D:  --*r*n*--  "
        L"     VSHADOW C: E:  --*r*n*--  "
        L"  --*r*n*--  "
        L" - List all shadow copies in the system:  --*r*n*--  "
        L"     VSHADOW -q  --*r*n*--  "
...
CommandLineParser::~CommandLineParser()
{
    ...
    if (m_bWaitForFinish)
    {
        ft.WriteLine(L"  --*r*n*--  Press <ENTER> to continue...");
