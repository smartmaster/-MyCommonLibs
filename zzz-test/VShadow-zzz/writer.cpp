...
void VssClient::GatherWriterMetadataToScreen()
{
	...
	for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
	{
		...
		D_INFO(0, L"  --*r*n*--  --[Writer %u]--  --*r*n*--  %s  --*r*n*--  ", iWriter, (LPCWSTR)(bstrXML));
	}

	D_INFO(0, L"--[end of data]--  --*r*n*--  ");
}
...
void VssClient::InitializeWriterMetadata()
{
	...
	// Enumerate writers
	m_writerList.resize(cWriters); //myCode 2013/04/04
	for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
	{
		// Get the metadata for this particular writer
		VSS_ID idInstance = GUID_NULL;
		...
		m_writerList[iWriter].Initialize(pMetadata); //myCode 2013/04/04
	}
}
...
void VssClient::InitializeWriterComponentsForRestore()
{
	...
	for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
	{
		...
		bool bFound = false;
		size_t Count = 0;
		m_writerComponentsForRestore.resize(m_writerList.size());
		for (unsigned i = 0; i < m_writerList.size(); i++)
		{
			...
			m_writerComponentsForRestore[Count] = m_writerList[i];
			++ Count;

			ft.WriteLine(L"- Writer %s is present in the Backup Components document and on the system. Considering for restore ...",
				m_writerComponentsForRestore[Count].name.c_str());

			// Adding components
			m_writerComponentsForRestore[Count].InitializeComponentsForRestore(pWriterComponents); //myCode 2013/04/04
		...
		m_writerComponentsForRestore.resize(Count);
...
void VssClient::ListWriterStatus()
{
	...
	for(unsigned iWriter = 0; iWriter < cWriters; iWriter++)
	{
		...
		ft.WriteLine(L"  --*r*n*--  "
			L"* WRITER \"%s\"  --*r*n*--  "
			L"   - Status: %d (%s)  --*r*n*--  " 
			L"   - Writer Failure code: 0x%08lx (%s)  --*r*n*--  " 
			L"   - Writer ID: " WSTR_GUID_FMT L"  --*r*n*--  "
			L"   - Instance ID: " WSTR_GUID_FMT L"  --*r*n*--  ",
...
void VssClient::PreRestore()
{
	...
	ft.WriteLine(L"  --*r*n*--  Sending the PreRestore event ...   --*r*n*--  ");
...
void VssClient::PostRestore()
{
	...
	ft.WriteLine(L"  --*r*n*--  Sending the PostRestore event ...   --*r*n*--  ");
...
void VssWriter::Initialize(IVssExamineWriterMetadata * pMetadata)
{
	...
	// Get exclude files
	excludedFiles.resize(cExcludeFiles);
	for(unsigned i = 0; i < cExcludeFiles; i++)
	{
		...
		excludedFiles[i].Initialize(pFileDesc, VSS_FDT_EXCLUDE_FILES); //myCode 2013/04/04
	}

	// Enumerate components
	components.resize(cComponents);
	for(unsigned iComponent = 0; iComponent < cComponents; iComponent++)
	{
		...
		components[iComponent].Initialize(name, pComponent); //myCode 2013/04/04
	}
...
void VssWriter::InitializeComponentsForRestore(IVssWriterComponentsExt * pWriterComponents)
{
	...
	for(unsigned iComponent = 0; iComponent < cComponents; iComponent++)
	{
		...
		// Add this component to the list of components
		components[iComponent].Initialize(name, pComponent);

		ft.WriteLine(L"- Found component available for restore: \"%s\"", components[iComponent].fullPath.c_str());
	}
}
...
void VssWriter::Print(bool bListDetailedInfo)
{
	...
	ft.WriteLine(L"  --*r*n*--  "
		...
		L"    - WriterId   = %s  --*r*n*--  "
		L"    - InstanceId = %s  --*r*n*--  "
		L"    - Supports restore events = %s  --*r*n*--  "
		L"    - Writer restore conditions = %s  --*r*n*--  "
		L"    - Restore method = %s  --*r*n*--  "
		L"    - Requires reboot after restore = %s  --*r*n*--  ",
		name.c_str(),
		id.c_str(),
		instanceId.c_str(),
...
void VssComponent::Initialize(wstring writerNameParam, IVssWMComponent * pComponent)
{
	...
	descriptors.resize(pInfo->cFileCount  + pInfo->cDatabases + pInfo->cLogFiles);
	size_t index = 0;
	for(unsigned i = 0; i < pInfo->cFileCount; i++)
	{
		...
		descriptors[index ++].Initialize(pFileDesc, VSS_FDT_FILELIST);
	}
	...
	for(unsigned i = 0; i < pInfo->cDatabases; i++)
	{
		...
		descriptors[index ++].Initialize(pFileDesc, VSS_FDT_DATABASE);
	}
	...
	for(unsigned i = 0; i < pInfo->cLogFiles; i++)
	{
		...
		descriptors[index ++].Initialize(pFileDesc, VSS_FDT_DATABASE_LOG);
	}
	

#ifdef VSS_SERVER
	// Get dependencies
	dependencies.resize(pInfo->cDependencies);
	for(unsigned i = 0; i < pInfo->cDependencies; i++)
	{
		...
		dependencies[i].Initialize(pDependency);
	}
#endif
...
void VssComponent::Print(bool bListDetailedInfo)
{
	...
	// Print writer identity information
	ft.WriteLine(L"    - Component \"%s\"  --*r*n*--  "
		L"       - Name: '%s'  --*r*n*--  "
		L"       - Logical Path: '%s'  --*r*n*--  "
		L"       - Full Path: '%s'  --*r*n*--  "
		L"       - Caption: '%s'  --*r*n*--  "
		L"       - Type: %s [%d]  --*r*n*--  "
		L"       - Is Selectable: '%s'  --*r*n*--  "
		L"       - Is top level: '%s'  --*r*n*--  "
		...
		)
{
	...
	alternatePath = BSTR2WString(bstrAlternate);
	isRecursive = bRecursive; //myCode 2013/04/04
	

	// Get the expanded path
	expandedPath.resize(MAX_PATH, L'\0');
	_VSHADOW_ASSERTE(bstrPath && bstrPath[0]);
