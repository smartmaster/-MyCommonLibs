...
//static const struct error_table error_table_data[29];
...
static const struct error_table error_table_data[] =
{
...
static NTSTATUS WINAPI SearchSingleTable(int index, DWORD winError)
{
	NTSTATUS status = -1;

	const struct error_table * et = &error_table_data[index];
	for (DWORD ii = et->start; ii < et->end; ++ii)
	{
		if (et->table[ii - et->start] == winError)
		{
			status = ii;
			break;
		}
...
NTSTATUS WINAPI SmlWinErrorToNtStatus(DWORD winError)
{
	...
	for (DWORD index = 0; index < _countof(error_table_data); ++index)
	{
		NTSTATUS statusTmp = SearchSingleTable(index, winError);
		if (-1 != statusTmp)
		{
			status = statusTmp;
			break;
...
EXTERN_C
ULONG WINAPI SmlNtStatusToWinError(NTSTATUS status)
{
	const struct error_table *table = error_table_data;
