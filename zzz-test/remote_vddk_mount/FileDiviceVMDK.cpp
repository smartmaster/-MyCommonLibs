...
HRESULT CFileDeviceVMDK::SetPointerEx(LONGLONG llDistanceToMove, LONGLONG* lpNewFilePointer, ULONG ulMoveMethod)
{
	...
	{
		PRINT_LINE("MoveMethod is not FILE_BEGIN [%d]", ulMoveMethod);
		hr = E_INVALIDARG;
		LEAVE_BLOCK(0);
	}
	...
	{
		PRINT_LINE("DistanceToMove is not sector size aligned [%I64d]", llDistanceToMove);
		hr = E_INVALIDARG;
		LEAVE_BLOCK(0);
	}
...
HRESULT CFileDeviceVMDK::Read(ULONG nNumberOfBytesToRead, ULONG* lpNumberOfBytesRead, BYTE* pBuffer)
{
	...
	{
		PRINT_LINE("NumberOfBytesToRead is not sector size aligned [%d]", nNumberOfBytesToRead);
		hr = E_INVALIDARG;
		LEAVE_BLOCK(0);
	}
	...
	if (VIX_FAILED(err))
	{
		...
		PRINT_LINE("VixDiskLib_Read" " failed with %I64d %s", err, errMsg);
		hr = E_FAIL;
		LEAVE_BLOCK(0);
	}
...
HRESULT CFileDeviceVMDK::Write(ULONG nNumberOfBytesToWrite, ULONG* lpNumberOfBytesWritten, CONST BYTE* pBuffer)
{
	...
	{
		PRINT_LINE("NumberOfBytesToWrite is not sector size aligned [%d]", nNumberOfBytesToWrite);
		hr = E_INVALIDARG;
		LEAVE_BLOCK(0);
	}
	...
	if (VIX_FAILED(err))
	{
		...
		PRINT_LINE("VixDiskLib_Write" " failed with %I64d %s", err, errMsg);
		hr = E_FAIL;
		LEAVE_BLOCK(0);
	}
...
HRESULT CFileDeviceVMDK::GetFileSize(LONGLONG * pFileSize)
{
	...
	if (-1LL == m_diskSize)
	{
		...
		if (VIX_FAILED(err))
		{
			...
			PRINT_LINE("VixDiskLib_GetInfo" " failed with %I64d %s", err, errMsg);
			hr = E_FAIL;
		}
		else
		{
