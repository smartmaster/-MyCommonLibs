...
HRESULT RemoteVddkMount(CONST MountParams & params, IVolumeOperationCallback * pVolCallback)
{
	...
	if (VIX_FAILED(err))
	{
		...
		PRINT_LINE("VixDiskLib_InitEx" " failed with %I64d %s", err, errMsg);
	...
	{
		char * errMsg = VixDiskLib_GetErrorText(err, NULL);
		PRINT_LINE("VixMntapi_Init" " failed with %I64d %s", err, errMsg);
		VixDiskLib_FreeErrorText(errMsg);
		LEAVE_BLOCK(0);
	}
	...
	{
		char * errMsg = VixDiskLib_GetErrorText(err, NULL);
		PRINT_LINE("VixDiskLib_ConnectEx" " failed with %I64d %s", err, errMsg);
		VixDiskLib_FreeErrorText(errMsg);
		LEAVE_BLOCK(0);
	}
	...
	for (size_t ii = 0; ii < params.diskPath.size(); ++ii)
	{
		...
		{
			char * errMsg = VixDiskLib_GetErrorText(err, NULL);
			PRINT_LINE("VixDiskLib_Open" " failed with %I64d %s" " %s", err, errMsg, diskPath.GetString());
			VixDiskLib_FreeErrorText(errMsg);
			break;
		}
	...
	{
		PRINT_LINE("VixDiskLib_Open" " - no VMDK opened, please check disk urls");
		hr = E_FAIL;
		LEAVE_BLOCK(0);
	}
	...
	{
		char * errMsg = VixDiskLib_GetErrorText(err, NULL);
		PRINT_LINE("VixMntapi_OpenDiskSet" " failed with %I64d %s", err, errMsg);
		VixDiskLib_FreeErrorText(errMsg);
		LEAVE_BLOCK(0);
	}
	...
	{
		char * errMsg = VixDiskLib_GetErrorText(err, NULL);
		PRINT_LINE("VixMntapi_GetDiskSetInfo" " failed with %I64d %s", err, errMsg);
		VixDiskLib_FreeErrorText(errMsg);
		//LEAVE_BLOCK(0);
	}
	...
	if (params.bGetOsInfo)
	{
		...
		{
			char * errMsg = VixDiskLib_GetErrorText(err, NULL);
			PRINT_LINE("VixMntapi_GetOsInfo" " failed with %I64d %s", err, errMsg);
			VixDiskLib_FreeErrorText(errMsg);
			//LEAVE_BLOCK(0);
		}
	...
	{
		char * errMsg = VixDiskLib_GetErrorText(err, NULL);
		PRINT_LINE("VixMntapi_GetVolumeHandles" " failed with %I64d %s", err, errMsg);
		VixDiskLib_FreeErrorText(errMsg);
		LEAVE_BLOCK(0);
	}
	...
	if (bFirstTimeMountPoint)
	{
		if (spFileLock)
		{
			PRINT_LINE("Begin to acqurie lock to mount");
			hr = spFileLock->LockEx(3000, (ULONG)(-2));
			if (FAILED(hr))
			{
				PRINT_LINE("failed to acquire lock to mount");
			}
			else
			{
				PRINT_LINE("Acquired lock to mount");
			}
		}
	...
	for (size_t ii = 0; ii < numberOfVolumes; ++ii)
	{
		...
		{
			char * errMsg = VixDiskLib_GetErrorText(err, NULL);
			PRINT_LINE("VixMntapi_MountVolume" " failed with %I64d %s", err, errMsg);
			VixDiskLib_FreeErrorText(errMsg);
		}
		else
		{
			...
			{
				char * errMsg = VixDiskLib_GetErrorText(err, NULL);
				PRINT_LINE("VixMntapi_GetVolumeInfo" " failed with %I64d %s", err, errMsg);
				VixDiskLib_FreeErrorText(errMsg);
				//LEAVE_BLOCK(0);
			}
			else
			{
				...
				if (!bRet)
				{
					...
					PRINT_LINE("DefineDosDeviceA" " failed with %d", hr);
	...
	if (params.bDismount)
	{
		...
		for (size_t ii = 0; ii < mountPoints.size(); ++ii)
		{
			...
			if (!bDVMP)
			{
				LastError = GetLastError();
				hr = HRESULT_FROM_WIN32(LastError);
				PRINT_LINE("DeleteVolumeMountPoint" " failed with %d", hr);
			}
		}
		...
		for (size_t ii = 0; ii < mountedVoluneHandle.size(); ++ii)
		{
			...
			{
				char * errMsg = VixDiskLib_GetErrorText(err, NULL);
				PRINT_LINE("VixMntapi_DismountVolume" " failed with %I64d %s", err, errMsg);
				VixDiskLib_FreeErrorText(errMsg);
				//LEAVE_BLOCK(0);
			}
		}
	...
	if (diskSetHandle)
	{
		...
		{
			char * errMsg = VixDiskLib_GetErrorText(err, NULL);
			PRINT_LINE("VixMntapi_CloseDiskSet" " failed with %I64d %s", err, errMsg);
			VixDiskLib_FreeErrorText(errMsg);
			//LEAVE_BLOCK(0);
		}
	}
	...
	for (size_t ii = 0; ii < diskHandles.size(); ++ii)
	{
		...
		{
			char * errMsg = VixDiskLib_GetErrorText(err, NULL);
			PRINT_LINE("VixDiskLib_Close" " failed with %I64d %s", err, errMsg);
			VixDiskLib_FreeErrorText(errMsg);
			//LEAVE_BLOCK(0);
		}
	}
	...
	if (connection)
	{
		...
		{
			char * errMsg = VixDiskLib_GetErrorText(err, NULL);
			PRINT_LINE("VixDiskLib_Disconnect" " failed with %I64d %s", err, errMsg);
			VixDiskLib_FreeErrorText(errMsg);
			//LEAVE_BLOCK(0);
		}
	}
