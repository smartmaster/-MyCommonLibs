...
public:
	...
	HRESULT CreateMember()
	{
		...
		for (INT ii = 0; ii < m_ElementCount; ++ ii)
		{
			...
			if (FAILED(hr))
			{
				D_SET_LAST_STATUS(hr, 0);
				D_API_ERR(0, TEXT("ElementType::CreateMember"), hr, TEXT(""));
		...
		else if (WAIT_OBJECT_0 + 2 == dwWait) //allocate
		{
			...
			*ppElement = AllocateNoLock();
			if (NULL == *ppElement)
			{
				...
				D_API_ERR(0, TEXT("AllocateNoLock"), hr, TEXT("Unexpectd error: all the elements have been used up"));
				LEAVE_BLOCK(0);
			}
		}
	...
	HRESULT MTFree(MY_TYPE * pElement)
	{
		...
		if (m_ElementPool.end() == iter)
		{
			hr = E_FAIL;
			D_SET_LAST_STATUS(hr, -1);
			D_API_ERR(0, TEXT("MTFree"), hr, TEXT("Unexpected error: free a non-exiting element"));
			LEAVE_BLOCK(0);
		}
		else if (FALSE == iter->second)
		{
			hr = E_FAIL;
			D_SET_LAST_STATUS(hr, -1);
			D_API_ERR(0, TEXT("MTFree"), hr, TEXT("Unexpected error: free a non-allocated element"));
			LEAVE_BLOCK(0);
		}
		else
		{
	...
	HRESULT MTPopFront(MY_TYPE ** ppElement)
	{
		...
		else if (WAIT_OBJECT_0 + 2 == dwWait) //pop front
		{
			...
			if (m_ElementQueue.empty())
			{
				hr = E_FAIL;
				D_SET_LAST_STATUS(hr, -1);
				D_API_ERR(0, TEXT("pop_front"), hr, TEXT("Unexpectd error: empty queue"));
				LEAVE_BLOCK(0);
			}
			else
			{
		...
		{
			hr = S_OK;
			D_INFO(0, TEXT("Provider operation finished"));
			LEAVE_BLOCK(0);
		}
		else if (WAIT_TIMEOUT == dwWait)
		{
