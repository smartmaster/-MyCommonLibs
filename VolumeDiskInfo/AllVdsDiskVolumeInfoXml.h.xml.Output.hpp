...
VOID CAllVdsDiskVolumeInfoXml::ToElement(LPCTSTR RootName, XmlElement & xmlElement) CONST
{
	xmlElement.m_Name = RootName;

	AllocateOneMore(xmlElement.m_Children);
	/*VectorStructToElement*/ContainerStructToElement(FIELD_NAME(m_pVdsDiskInfos), m_pVdsDiskInfos, Last(xmlElement.m_Children));

	AllocateOneMore(xmlElement.m_Children);
	/*VectorStructToElement*/ContainerStructToElement(FIELD_NAME(m_pVdsVolumeInfos), m_pVdsVolumeInfos, Last(xmlElement.m_Children));
...
////////////////////////////RRRRRR BEGIN//////////////////////////////////////////////
VOID CAllVdsDiskVolumeInfoXml::FromElement(LPCTSTR RootName, CONST XmlElement & xmlElement)
{
	...
	for (size_t ik9856i = 0; ik9856i < xmlElement.m_Children.size(); ++ ik9856i )
	{
		...
		else if (0 == Child_jfs654.m_Name.CompareNoCase(FIELD_NAME(m_pVdsDiskInfos)))
		{
			/*ElementToVectorStruct*/ElementToContainerStruct(FIELD_NAME(m_pVdsDiskInfos), m_pVdsDiskInfos, Child_jfs654);
		}
		else if (0 == Child_jfs654.m_Name.CompareNoCase(FIELD_NAME(m_pVdsVolumeInfos)))
		{
			/*ElementToVectorStruct*/ElementToContainerStruct(FIELD_NAME(m_pVdsVolumeInfos), m_pVdsVolumeInfos, Child_jfs654);
		}
	}
}
VOID CAllVdsDiskVolumeInfoXml::ToString( LPCTSTR RootName, CString & strXml ) CONST
{
...
HRESULT CAllVdsDiskVolumeInfoXml::FromString( LPCTSTR RootName, CONST CString & strXml )
{
	XmlElement xmlElement;
	INT Start = 0;
	HRESULT hr = S_OK;
...
HRESULT CAllVdsDiskVolumeInfoXml::ToStream( LPCTSTR RootName, IFileDevice * pIFileDevice ) CONST
{
	HRESULT hr = S_OK;
	BEGIN_BLOCK(0);
	CString strXml;
...
HRESULT CAllVdsDiskVolumeInfoXml::FromStream( LPCTSTR RootName, IFileDevice * pIFileDevice )
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;
	BEGIN_BLOCK(0);
...
HRESULT CAllVdsDiskVolumeInfoXml::ToFile( LPCTSTR RootName, LPCTSTR pFileName ) CONST
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;
	BEGIN_BLOCK(0);
...
HRESULT CAllVdsDiskVolumeInfoXml::FromFile( LPCTSTR RootName, LPCTSTR pFileName )
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;
	BEGIN_BLOCK(0);
