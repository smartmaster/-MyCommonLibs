...
INT CVolumeHelper::GetAllVolumeGuid( vector<CString> & VolumeGuid )
{
	...
	do 
	{
		...
		if (INVALID_HANDLE_VALUE == hFindFirstVolume)
		{
			...
			nStatus = dwLastError;
			D_API_ERR(0, /*DRNPOS,*/ TEXT("FindFirstVolume"), dwLastError, TEXT("")); /*0002*/
			break;
		}
...
INT CVolumeHelper::GetVolumePathFromGuid( LPCTSTR szVolumeGuid, vector<CString> & Path )
{
	...
	do 
	{
		...
		if (NULL == szVolumeGuid)
		{
			...
			D_API_ERR(0, /*DRNPOS,*/ TEXT(__FUNCTION__), E_INVALIDARG, TEXT("")); /*0010*/
			break;
		}
		...
		if (!bGetVolumePathNamesForVolumeName)
		{
			...
			{
				D_API_ERR(999, /*DRNPOS,*/ TEXT("GetVolumePathNamesForVolumeName"), dwLastError, TEXT("")); /*0002*/
				nStatus = dwLastError;
				break;
			}
			...
			{
				dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
				D_API_ERR(0, /*DRNPOS,*/ TEXT("GetVolumePathNamesForVolumeName"), dwLastError, TEXT("")); /*0002*/
				nStatus = dwLastError;
				break;
			}
			...
			if (nStatus)
			{
				D_INFO(0, /*DRNPOS,*/ TEXT("CUtilityHelper::MultiStringToStringVector [ERROR]"));
				break;
			}
		}
...
INT CVolumeHelper::GetVolumeRootAndGuidFromPath( LPCTSTR szPath, CString & strRoot, CString & strGuid )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("GetVolumePathName"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
		strRoot = szBuffer;
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("GetVolumeNameForVolumeMountPoint"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
		strGuid = szBuffer;
...
INT CVolumeHelper::GetVolumeMountPoint( LPCTSTR szRootPathName, vector<CString> & MountPoint )
{
	...
	do 
	{
		...
		if (INVALID_HANDLE_VALUE == hFindFirstVolumeMountPoint)
		{
			...
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			nStatus = dwLastError;
			D_API_ERR(999, /*DRNPOS,*/ TEXT("FindFirstVolume"), dwLastError, TEXT("")); /*0002*/
			break;
		}
...
INT CVolumeHelper::GetVolumeInfo( LPCTSTR lpRootPathName, CVolumeInfomation & volumeInfo )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(999, /*DRNPOS,*/ TEXT("GetVolumeInformation"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
...
HANDLE CVolumeHelper::CreateHandle( LPCTSTR szVolume, DWORD dwDesiredAccess )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("CreateFile"), dwLastError, szVolume);
			nStatus = dwLastError;
			break;
		}
...
INT CVolumeHelper::FSCTL_GET_NTFS_VOLUME_DATA_Helper(HANDLE hDevice, NTFS_VOLUME_DATA_BUFFER & NtfsVolumeDataBuffer )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("FSCTL_GET_NTFS_VOLUME_DATA"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
...
INT CVolumeHelper::FSCTL_GET_RETRIEVAL_POINTER_BASE_Helper( HANDLE hDevice, RETRIEVAL_POINTER_BASE & RetrievalPointerBase )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("FSCTL_GET_RETRIEVAL_POINTER_BASE"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
	...
	do 
	{
		...
		do 
		{
			...
			else
			{
				...
				{
					D_API_ERR(0, /*DRNPOS,*/ TEXT("FSCTL_GET_RETRIEVAL_POINTERS"), dwLastError, TEXT("")); /*0002*/
					nStatus = dwLastError;
					break;
				}
	...
	do 
	{
		...
		do 
		{
			...
			else
			{
				...
				if (ERROR_MORE_DATA != dwLastError)
				{
					D_API_ERR(0, /*DRNPOS,*/ TEXT("FSCTL_GET_VOLUME_BITMAP"), dwLastError, TEXT("")); /*0002*/
					nStatus = dwLastError;
					break;
				}
...
INT CVolumeHelper::FSCTL_QUERY_FILE_SYSTEM_RECOGNITION_Helper( HANDLE hDevice, FILE_SYSTEM_RECOGNITION_INFORMATION & FileSystemRecognitionInformation )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("FSCTL_QUERY_FILE_SYSTEM_RECOGNITION"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
...
INT CVolumeHelper::IOCTL_VOLUME_GET_GPT_ATTRIBUTES_Helper( HANDLE hDevice, VOLUME_GET_GPT_ATTRIBUTES_INFORMATION & GptAttributes )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("IOCTL_VOLUME_GET_GPT_ATTRIBUTES"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
	...
	do 
	{
		...
		do 
		{
			...
			else
			{
				...
				if (ERROR_MORE_DATA != dwLastError)
				{
					D_API_ERR(999, /*DRNPOS,*/ TEXT("IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS"), dwLastError, TEXT("")); /*0002*/
					nStatus = dwLastError;
					break;
				}
	...
	do 
	{
		...
		do 
		{
			...
			else
			{
				...
				if (ERROR_MORE_DATA != dwLastError)
				{
					D_API_ERR(0, /*DRNPOS,*/ TEXT("IOCTL_VOLUME_LOGICAL_TO_PHYSICAL"), dwLastError, TEXT("")); /*0002*/
					nStatus = dwLastError;
					break;
				}
...
INT CVolumeHelper::IOCTL_VOLUME_PHYSICAL_TO_LOGICAL_Helper( HANDLE hDevice, VOLUME_PHYSICAL_OFFSET & VolumePhysicalOffset, VOLUME_LOGICAL_OFFSET & VolumeLogicalOffset )
{
	...
	do 
	{
		...
		{
			dwLastError = GetLastError(); D_SET_LAST_STATUS(dwLastError, -1);
			D_API_ERR(0, /*DRNPOS,*/ TEXT("IOCTL_VOLUME_PHYSICAL_TO_LOGICAL"), dwLastError, TEXT("")); /*0002*/
			nStatus = dwLastError;
			break;
		}
...
INT TestVolumeBlockOffset(LPCTSTR volumeName, LONG blockSizeKB, LONG count) //no closeHandle or feeMem, do not use for production
{
	HANDLE hDeviceRaw = CVolumeHelper::CreateHandle(volumeName, GENERIC_READ);
	LONGLONG blockSize = (blockSizeKB << 10);

	VOLUME_LOGICAL_OFFSET VolumeLogicalOffset = { 0 };
	VOLUME_PHYSICAL_OFFSETS * pVolumePhysicalOffsets = NULL;

	VOLUME_DISK_EXTENTS * pVolumeDiskExtents = NULL;
	CVolumeHelper::IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS_Helper(hDeviceRaw, &pVolumeDiskExtents);

	_ftprintf_s(stdout, TEXT("Volume: %s disk extents") TEXT("\r\n"), volumeName);
	const LONG WIDTH[] = { 8, 16, 16, 16 };
	_ftprintf_s(stdout, TEXT("%*s|%*s|%*s|%*s|") TEXT("\r\n"), WIDTH[0], TEXT(""), WIDTH[1], TEXT("DiskNumber"), WIDTH[2], TEXT("Offset"), WIDTH[3], TEXT("Length"));
	for (DWORD ii = 0; ii < pVolumeDiskExtents->NumberOfDiskExtents; ++ ii)
	{
		DISK_EXTENT & diskExt = pVolumeDiskExtents->Extents[ii];
		_ftprintf_s(stdout, TEXT("%*s|%*d|%*I64d|%*I64d|") TEXT("\r\n"), WIDTH[0], TEXT(""), WIDTH[1], diskExt.DiskNumber, WIDTH[2], diskExt.StartingOffset.QuadPart, WIDTH[3], diskExt.ExtentLength.QuadPart);
	}
	

	const LONG WIDTH2[] = { 16, 16, 16 };
	_ftprintf_s(stdout, TEXT("|%*s|%*s|%*s|") TEXT("\r\n"), WIDTH2[0], TEXT("LogicalBlock"), WIDTH2[1], TEXT("DiskNumber"), WIDTH2[2], TEXT("PhisycalBlock"));
	for (LONG ii = 0; ii < count; ++ ii)
	{
		VolumeLogicalOffset.LogicalOffset = blockSize * ii;
		CVolumeHelper::IOCTL_VOLUME_LOGICAL_TO_PHYSICAL_Helper(hDeviceRaw, VolumeLogicalOffset, &pVolumePhysicalOffsets);
		VOLUME_PHYSICAL_OFFSET & po = pVolumePhysicalOffsets->PhysicalOffset[0];
		DISK_EXTENT * pde = nullptr;
		for (DWORD jj = 0; jj < pVolumeDiskExtents->NumberOfDiskExtents; ++jj)
		{
			DISK_EXTENT & diskExt = pVolumeDiskExtents->Extents[jj];
			if (po.DiskNumber == diskExt.DiskNumber)
			{
				pde = &diskExt;
				break;
			}
		}
		_ftprintf_s(stdout, TEXT("|%*d|%*d|%*I64d|") TEXT("\r\n"), WIDTH2[0], ii, WIDTH2[1], po.DiskNumber, WIDTH2[2], (po.Offset - pde->StartingOffset.QuadPart)/blockSize);
		
		{
			CHeapPtr<BYTE> apTemp;
			apTemp.Attach((BYTE*)pVolumePhysicalOffsets);
		}
	}
