...
protected:
	...
	IFileDevice * m__pFileDeviceVolume;
...
private:
	...
	BOOL ReadVirtualClustersHelper(ULONGLONG vcn, DWORD clusters,
		void *bufv, DWORD bufLen, DWORD *actural, BOOL & isSparsed); //<sonmi01>2015-6-4 ###???
	HRESULT CacheReadVirtualCompressedBlock(LONGLONG vcn, void *bufv, DWORD bufLen, DWORD *actural); //<sonmi01>2015-6-4 ###???

	BOOL ReadVirtualClusters(ULONGLONG vcn, DWORD clusters,
		void *bufv, DWORD bufLen, DWORD *actural, BOOL & isSparsed);
	

private:
	CVMemPtr<BYTE> m_helperCache;
	CVMemPtr<BYTE> m_dataCache;
	LONGLONG m_cachedStartVCN;
	LONG m_cacheSize;
	CAutoPtr<NTDllAPITable> m_ntdllApi;
...
public:
	...
	ULONGLONG GetParentReference() CONST;
...
public:
	...
	virtual ~CAttr_Data()
	{
		//DEBUG_TRACE0("CAttr_Data dtor"); //<sonmi01>2014-4-20 ###???
	}
};	// CAttr_Data
...
public:
	...
	//<sonmi01>2015-4-23 ###???
	template<class T>
	T * Convert_INDEX_ENTRY()
	{
		return (T *)(m_IndexEntry);
	}
};	// CIndexEntry
...
private:
	...
	LONG m_IndexVirtualClusterSize; //<sonmi01>2014-4-18 ###???

	void ParseIndexEntries();

public:
	 BOOL IsFileName() CONST;
	 LONG CalcIndexRootVirtualClusterSize(); //<sonmi01>2014-4-18 ###???;
	 LONG GetIndexRootIndexBufferSize() CONST;
};	// CAttr_IndexRoot
...
public:
	 ...
	 BOOL ParseIndexBlock(CONST ULONGLONG vcn, CIndexBlock &ibClass, CONST LONG InedxRootVirtualClusterSize, CONST LONG IndexRootIndexBufferSize); //<sonmi01>2014-4-18 ###???
};	// CAttr_IndexAlloc
...
CAttr_Bitmap<TYPE_RESIDENT>::~CAttr_Bitmap()
{
	...
	//DEBUG_TRACE0("CAttr_Bitmap dtor"); //<sonmi01>2014-4-20 ###???
}
...
CAttr_AttrList<TYPE_RESIDENT>::CAttr_AttrList(CONST ATTR_HEADER_COMMON *ahc, CONST CFileRecord *fr) : TYPE_RESIDENT(ahc, fr)
{
	...
	std::set<ULONGLONG> UniqFileRefNo; //<sonmi01>2014-4-20 ###???
		...
		len == sizeof(ATTR_ATTRIBUTE_LIST))
	{
		...
		ULONGLONG recordRef = alRecord.m_BaseRef & NTFS_FILE_REF_MASK;
		if (recordRef != fr->m_FileReference /*&& UniqFileRefNo.end() == UniqFileRefNo.find(recordRef)*/)	// Skip contained attributes
		{
			//UniqFileRefNo.insert(recordRef); //<sonmi01>2014-4-20 ###??? //<sonmi01>2015-11-20 ###???
			...
			if (am & fr->m_AttrMask)	// Skip unwanted attributes
			{
				...
				//frnew->m_AttrMask = am;
				frnew->SetAttrMask(MASK_ALL); //<sonmi01>2014-4-20 ###???
				...
				// Attribute list entry can contain reference to other file’s MFT segment.
				// So, Skipping this record if it contains $10 attribute.
				CONST CAttrBase * abStdInfo = frnew->FindFirstAttr(ATTR_TYPE_STANDARD_INFORMATION);
				if (NULL == abStdInfo) //<sonmi01>2014-4-20 ###???
				{
...
{
	//DEBUG_TRACE0("CAttr_AttrList dtor"); //<sonmi01>2014-4-20 ###???
}
