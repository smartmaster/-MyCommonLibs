...
BOOL CAttrNonResident::ParseDataRun()
{
	...
	LONGLONG LCN = 0;
	//ULONGLONG VCN = 0;
	ULONGLONG VCN = m_AttrHeaderNR->m_StartVCN; //<sonmi01>2014-4-21 ###???
	...
	while (*dataRun)
	{
		if (PickData(&dataRun, &length, &LCNOffset))
		{
			...
			//DEBUG_TRACE2("Data length = %I64d clusters, LCN = %I64d", length, LCN); //<sonmi01>2014-4-20 ###???
			if (0 == LCNOffset)
			{
			...
			if (dr->m_StartVCN >= m_AttrHeaderNR->m_StartVCN &&
				dr->m_LastVCN <= m_AttrHeaderNR->m_LastVCN &&
				(dr->m_LastVCN - dr->m_StartVCN) <= (m_AttrHeaderNR->m_LastVCN - m_AttrHeaderNR->m_StartVCN)) //<sonmi01>2014-4-21 ###???
			{
				m_DataRunList.InsertEntry(dr);
			}
			else
			{
				DEBUG_TRACE4("DataRun decode error: VCN exceeds bound, dr->m_StartVCN=%I64d, dr->m_LastVCN=%I64d, m_AttrHeaderNR->m_StartVCN=%I64d, m_AttrHeaderNR->m_LastVCN=%I64d ", dr->m_StartVCN, dr->m_LastVCN, m_AttrHeaderNR->m_StartVCN, m_AttrHeaderNR->m_LastVCN);
...
BOOL CAttrNonResident::ReadClusters(void *buf, DWORD clusters, LONGLONG lcn)
{
	...
	else
	{
		...
		{
			//DEBUG_TRACE2("Successfully read %u clusters from LCN %I64d", clusters, lcn); //<sonmi01>2014-4-20 ###???
			return TRUE;
		}
		else
		{
...
BOOL CAttrNonResident::ReadVirtualClustersHelper(ULONGLONG vcn, DWORD clusters,
	void *bufv, DWORD bufLen, DWORD *actural, BOOL & isSparsed) //<sonmi01>2015-6-4 ###???
{
	_ASSERT(bufv);
	_ASSERT(clusters);
	...
	*actural = 0;
	...
	while(dr)
	{
		if (vcn>=dr->m_StartVCN && vcn<=dr->m_LastVCN)
		{
			...
			BOOL bRead = TRUE; //<sonmi01>2015-6-4 ###???
			if (-1LL == dr->m_LCN)
			{
				isSparsed = TRUE;
				ZeroMemory(buf, clustersToRead*m__ClusterSize);
			}
			else
			{
				bRead = ReadClusters(buf, clustersToRead, dr->m_LCN + (vcn - dr->m_StartVCN));
			}

			if (bRead/*ReadClusters(buf, clustersToRead, dr->m_LCN+(vcn-dr->m_StartVCN))*/)
			{
...
HRESULT CAttrNonResident::CacheReadVirtualCompressedBlock(LONGLONG vcn, void *bufv, DWORD bufLen, DWORD *acturalRead) //<sonmi01>2015-6-4 ###???
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;

	CONST LONG COMPRESSED_CLUSER_COUNT = 16;

	BEGIN_BLOCK(0);
	//////////////////////////////////////////////////////////////////////////
	if (nullptr == m_dataCache.m_pData)
	{
		m_cacheSize = m__ClusterSize * COMPRESSED_CLUSER_COUNT;
		m_dataCache.Allocate(m_cacheSize);
		if (nullptr == m_dataCache.m_pData)
		{
			hr = E_OUTOFMEMORY;
	...
	if (nullptr == m_helperCache.m_pData)
	{
		m_cacheSize = m__ClusterSize * COMPRESSED_CLUSER_COUNT;
		m_helperCache.Allocate(m_cacheSize);
		if (nullptr == m_helperCache.m_pData)
		{
			hr = E_OUTOFMEMORY;
			D_API_ERR(0, TEXT("Allocate"), hr, TEXT(""));
	...
	//////////////////////////////////////////////////////////////////////////
	DWORD toReadSize = bufLen > m__ClusterSize ? m__ClusterSize : bufLen;
	LONGLONG byteOffset = vcn % COMPRESSED_CLUSER_COUNT * m__ClusterSize;

	//////////////////////////////////////////////////////////////////////////
	LONGLONG startVCN = AtlAlignDown(vcn, COMPRESSED_CLUSER_COUNT);
	if (startVCN == m_cachedStartVCN)
	{
		CopyMemory(bufv, m_dataCache.m_pData + byteOffset, toReadSize);
		*acturalRead = toReadSize;
		LEAVE_BLOCK(0); //already read
	}

	//////////////////////////////////////////////////////////////////////////
	DWORD actural = 0;
	BOOL isSparsed = FALSE;
	BOOL bREad = ReadVirtualClustersHelper(
		startVCN,//ULONGLONG vcn, 
		COMPRESSED_CLUSER_COUNT,//DWORD clusters,
		m_helperCache.m_pData,//void *bufv, 
		m_cacheSize,//DWORD bufLen, 
		&actural,//DWORD *actural
		isSparsed
		);
	if (!bREad)
	{
		hr = E_FAIL;
		D_API_ERR(0, TEXT("ReadVirtualClusters"), hr, TEXT(""));
		LEAVE_BLOCK(0);
	}

	if (isSparsed)
	{
		if (nullptr == m_ntdllApi.m_p)
		{
			m_ntdllApi.Attach(new NTDllAPITable());
			m_ntdllApi->Init();
		}
		//////////////////////////////////////////////////////////////////////////
		ULONG FinalUncompressedSize = 0;
		NTSTATUS ntStatus = STATUS_SUCCESS;
		if (nullptr != m_ntdllApi.m_p && nullptr != m_ntdllApi->RtlDecompressBuffer)
		{
			ntStatus = m_ntdllApi->RtlDecompressBuffer(
				COMPRESSION_FORMAT_LZNT1,//IN USHORT CompressionFormat,
				m_dataCache.m_pData,//OUT PUCHAR UncompressedBuffer,
				m_cacheSize,//IN ULONG UncompressedBufferSize,
				m_helperCache.m_pData,//IN PUCHAR CompressedBuffer,
				m_cacheSize,//IN ULONG CompressedBufferSize,
				&FinalUncompressedSize//OUT PULONG FinalUncompressedSize
				);
		}
		else
		{
		...
		if (!NT_SUCCESS(ntStatus))
		{
			...
			D_API_ERR(0, TEXT("DecompressBuffer"), hr, TEXT(""));
			LEAVE_BLOCK(0);
		}
	}
	else
	{
		CopyMemory(m_dataCache.m_pData, m_helperCache.m_pData, actural);
	}
...
BOOL CAttrNonResident::ReadVirtualClusters(ULONGLONG vcn, DWORD clusters, void *bufv, DWORD bufLen, DWORD *actural, BOOL & isSparsed)
{
	BOOL bRead = TRUE;
	HRESULT hr = S_OK;
	
	if (m_FileRecord->IsCompressed())
	{
		*actural = 0;
		LONG buffOffset = 0;
		for (LONG ii = 0; ii < clusters; ++ii)
		{
			DWORD curretnActural = 0;
			hr = CacheReadVirtualCompressedBlock(vcn + ii, ((BYTE*)bufv) + buffOffset, bufLen - buffOffset, &curretnActural);
			if (FAILED(hr))
			{
				bRead = FALSE;
				break;
			...
			*actural += curretnActural;
			buffOffset += m__ClusterSize;
		}
	}
	else
	{
		bRead = ReadVirtualClustersHelper(vcn, clusters,
			bufv, bufLen, actural, isSparsed);
	}
	return bRead;
}
...
BOOL CAttrNonResident::ReadData(CONST ULONGLONG offset, void *bufv, DWORD bufLen, DWORD *actural) CONST
{
	...
	BOOL isSparsed = FALSE; //<sonmi01>2015-6-4 ###???
	*actural = 0;
	...
	{
		DEBUG_TRACE2("offset > RealSize, %I64u,  %I64u", offset, m_AttrHeaderNR->m_RealSize);
		//return FALSE;
	}
		
	if (m_AttrHeaderNR->m_RealSize && (offset + bufLen) > m_AttrHeaderNR->m_RealSize) //<sonmi01>2014-4-21 ###???
	...
	if (startBytes != m__ClusterSize)
	{
		...
		if (((CAttrNonResident*)this)->ReadVirtualClusters(startVCN, 1, m_UnalignedBuf, m__ClusterSize, &len, isSparsed)
			&& len == m__ClusterSize)
		{
	...
	if (alignedClusters)
	{
		...
		if (((CAttrNonResident*)this)->ReadVirtualClusters(startVCN, alignedClusters, buf, alignedSize, &len, isSparsed)
			&& len == alignedSize)
		{
...
{
	//DEBUG_TRACE0("CAttr_StdInfo dtor"); //<sonmi01>2014-4-20 ###???
}
...
//<sonmi01>2015-4-24 ###???
ULONGLONG CNtfsFileName::GetParentReference() CONST
{
	return m_FileName ? (m_FileName->m_ParentRef & NTFS_FILE_REF_MASK) : -1LL;
}
...
{
	//DEBUG_TRACE0("CAttr_FileName dtor"); //<sonmi01>2014-4-20 ###???
}
...
{
	//DEBUG_TRACE0("CAttr_VolInfo dtor"); //<sonmi01>2014-4-20 ###???
}
...
{
	//DEBUG_TRACE0("CAttr_VolName dtor"); //<sonmi01>2014-4-20 ###???
...
CIndexEntry::~CIndexEntry()
{
	...
	//DEBUG_TRACE0("CIndexEntry dtor"); //<sonmi01>2014-4-20 ###???
}
...
{
	if (m_IndexEntry)
	return m_IndexEntry->m_FileReference & NTFS_FILE_REF_MASK;
	else
		return (ULONGLONG)-1;
}
...
CIndexBlock::~CIndexBlock()
{
	//DEBUG_TRACE0("IndexBlock dtor"); //<sonmi01>2014-4-20 ###???
...
CAttr_IndexRoot::CAttr_IndexRoot(CONST ATTR_HEADER_COMMON *ahc, CONST CFileRecord *fr): 
CAttrResident(ahc, fr),
m_IndexVirtualClusterSize(-1) //<sonmi01>2014-4-18 ###???
{
...
{
	//DEBUG_TRACE0("CAttr_IndexRoot dtor"); //<sonmi01>2014-4-20 ###???
}
...
LONG CAttr_IndexRoot::CalcIndexRootVirtualClusterSize() //<sonmi01>2014-4-18 ###???
{
	if (m_IndexVirtualClusterSize <= 0)
	{
		LONG CluserCount = (LONG)(INT8)(m_IndexRoot->m_ClustersPerIB);
		if (CluserCount < 0)
		{
			CluserCount = (1 << (-CluserCount));
		}

		if (m_IndexRoot->m_IBSize && CluserCount)
		{
			m_IndexVirtualClusterSize = (m_IndexRoot->m_IBSize / CluserCount);
		}

		DEBUG_TRACE4("m_IndexRoot->m_ClustersPerIB=%d, m_IndexRoot->m_IBSize=%d, CluserCount=%d, m_IndexVirtualClusterSize=%d", (LONG)(INT8)(m_IndexRoot->m_ClustersPerIB), m_IndexRoot->m_IBSize, CluserCount, m_IndexVirtualClusterSize);
	}

	if (m_IndexVirtualClusterSize <= 0)
	{
		m_IndexVirtualClusterSize = m_IndexRoot->m_IBSize;
		DEBUG_TRACE4("Adjusted - m_IndexRoot->m_ClustersPerIB=%d, m_IndexRoot->m_IBSize=%d, CluserCount=%d, m_IndexVirtualClusterSize=%d", (LONG)(INT8)(m_IndexRoot->m_ClustersPerIB), m_IndexRoot->m_IBSize, -2, m_IndexVirtualClusterSize);
	}


	return m_IndexVirtualClusterSize;
}

LONG CAttr_IndexRoot::GetIndexRootIndexBufferSize() CONST
{
	return m_IndexRoot->m_IBSize;
}
...
{
	//DEBUG_TRACE0("CAttr_IndexAlloc dtor"); //<sonmi01>2014-4-20 ###???
}
...
// ibClass holds the parsed Index Entries
 BOOL CAttr_IndexAlloc::ParseIndexBlock(CONST ULONGLONG vcn, CIndexBlock &ibClass, CONST LONG InedxRootVirtualClusterSize, CONST LONG IndexRootIndexBufferSize) //<sonmi01>2014-4-18 ###???
{
	 ATLASSERT(InedxRootVirtualClusterSize > 0 && IndexRootIndexBufferSize > 0);

	 LONG TempIndexBlockSize = IndexRootIndexBufferSize;
	 if (TempIndexBlockSize <= 0)
	 {
		 TempIndexBlockSize = m__IndexBlockSize;
	 }

	ULONGLONG vFileOffset = vcn * InedxRootVirtualClusterSize;
	ULONGLONG vFileSize = m_IndexBlockCount * TempIndexBlockSize;
	if (vFileOffset >= vFileSize)	// Bounds check
	{
		//return FALSE;
		DEBUG_TRACE2("Out of bounds - vFileOffset=%I64d, vFileSize=%I64d", vFileOffset, vFileSize);
	}

	// Allocate buffer for a single Index Block
	INDEX_BLOCK *ibBuf = ibClass.AllocIndexBlock(TempIndexBlockSize);

	// Sectors Per Index Block
	DWORD sectors = TempIndexBlockSize / m__SectorSize;

	// Read one Index Block
	DWORD len;
	if (ReadData(vFileOffset/*vcn*m__ClusterSize*//*m__IndexBlockSize*/, ibBuf, TempIndexBlockSize, &len) &&
		len == TempIndexBlockSize)
	{
	...
	{
		DEBUG_TRACE2("cannot read at offset %I64d, ReadLen=%d", vFileOffset, len);
		return FALSE;
	}
