...
class CAutoProvPointer
{
public:
	CAutoProvPointer(VSS_PROVIDER_PROP * ptr) : m_ptr(ptr) {};
	~CAutoProvPointer() { /*::VssFreeSnapshotProperties(m_ptr);*/ }
private:
	VSS_PROVIDER_PROP * m_ptr;
};
...
//<sonmi01>2014-11-21 ###???
inline wstring & BSTR2WStringRef(wstring & str, BSTR bstr)
{
	(bstr == NULL) ? str = (L"") : str = (bstr);
	return str;
}
...
inline bool IsEqual(const wstring & str1, const wstring & str2) //<sonmi01>2014-10-10 ###???
{
...
inline bool FindStringInList(const wstring & str, const vector<wstring> & stringList) //<sonmi01>2014-10-10 ###???
{
...
inline bool IsVolume(wstring volumePath)
{
    ...
    _VSHADOW_ASSERTE(volumePath.length() > 0);
...
inline wstring GetUniqueVolumeNameForMountPoint(wstring mountPoint)
{
    ...
    _VSHADOW_ASSERTE(mountPoint.length() > 0);
...
inline wstring GetUniqueVolumeNameForPath(wstring path, bool bIsBackup=false)
{
    ...
    _VSHADOW_ASSERTE(path.length() > 0);
...
inline wstring GetDeviceForVolumeName(wstring volumeName)
{
    ...
    _VSHADOW_ASSERTE(wcslen(volumeName.c_str()) > 0);
...
inline wstring ReadFileContents(const wstring & fileName)
{
    FunctionTracer ft(DBG_INFO);
