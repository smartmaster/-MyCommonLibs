...
void FunctionTracer::WriteLine(LPCTSTR format, ...)
{
    wstring buffer;
    VPRINTF_VAR_PARAMS(buffer, format);

	MY_WPRINTF(L"%s\r\n", buffer.c_str()); //<sonmi01>2014-10-21 ###???
	D_INFO(0, L"%s", buffer.c_str());
...
// Tracing routine
// Can throw HRESULT on invalid formats
void FunctionTracer::Trace(const wstring & file, int line, const wstring & functionName, LPCTSTR format, ...)
{
    if (m_traceEnabled)
    {
        wstring buffer;
		...
		MY_WPRINTF(L"[[%40s @ %10s:%4d]] %s\r\n", functionName.c_str(), fileName.c_str(), line, buffer.c_str()); //<sonmi01>2014-10-21 ###???
		D_INFO(0, L"[[%40s @ %10s:%4d]] %s", functionName.c_str(), fileName.c_str(), line, buffer.c_str());
    }
}


// Constructor
FunctionTracer::FunctionTracer(const wstring & fileName, INT lineNumber, const wstring & functionName) :
