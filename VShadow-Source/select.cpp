...
void VssClient::CheckSelectedWriterStatus(wstring bcdFileName) //<sonmi01>2014-11-21 ###???
{
    FunctionTracer ft(DBG_INFO);
    ...
    for(unsigned iWriter = 0; iWriter < cWriters; iWriter++)
    {
        ...
        switch(eWriterStatus)
        {
			...
			default:
				...
				{
					CComBSTR bstrXMLTmp;
					VSS_ID idInstanceTmp = GUID_NULL;
					CComPtr<IVssExamineWriterMetadata> pMetadataTmp;
					CHECK_COM(m_pVssObject->GetWriterMetadata(iWriter, &idInstanceTmp, &pMetadataTmp));
					CHECK_COM(pMetadataTmp->SaveAsXML(&bstrXMLTmp));


					// Save the XML string to the file
					CString strFileNameTmp = bcdFileName.c_str();
					strFileNameTmp.AppendFormat(TEXT(".%03d.%s.xml"), iWriter, (LPCTSTR)bstrWriterName);
					wstring strXMLTmp;
					WriteFile(strFileNameTmp.GetString(), BSTR2WStringRef(strXMLTmp, bstrXMLTmp)); //<sonmi01>2014-11-21 ###???
				}
				continue;
        }
