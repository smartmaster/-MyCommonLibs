    ...
    )
{
	...
	//m_bcdFile = outputXmlFile; //<sonmi01>2015-6-2 ###???
    ...
    {
        ft.WriteLine(L"List of created shadow copies: ");
		QuerySnapshotSet(m_latestSnapshotSetID, outputXmlFile); //<sonmi01>2015-6-2 ###???
    }
}
...
void VssClient::PrepareForBackup()
{
	...
	CheckSelectedWriterStatus(L""); //<sonmi01>2015-6-2 ###???
}
...
void VssClient::AddToSnapshotSet(vector<wstring> volumeList)
{
    ...
    _VSHADOW_ASSERTE(m_latestSnapshotIdList.size() == 0);
	
	std::vector<MyProviderInfo> vecPI;
	QueryProviders(vecPI);
    ...
    for (unsigned i = 0; i < volumeList.size(); i++)
    {
		...
		bool chooseProvider = false;
		if (chooseProvider) //use default provider
		{
			for (int ii = 0; ii < vecPI.size(); ++ii)
			{
				BOOL bSupported = false;
				HRESULT hrIVS = m_pVssObject->IsVolumeSupported(vecPI[ii].m_ProviderId, (LPWSTR)volume.c_str(), &bSupported);
				if (SUCCEEDED(hrIVS) && bSupported)
				{
					providerID = vecPI[ii].m_ProviderId;
					ft.WriteLine(L"Use provider [%s, %s] for Volume [%s]",
						Guid2WString(providerID).c_str(),
						vecPI[ii].m_pwszProviderName.GetString(),
						volume.c_str());
					break;
				}
			}
        ...
        CHECK_COM(m_pVssObject->AddToSnapshotSet((LPWSTR)volume.c_str(), providerID, &SnapshotID));
...
void VssClient::DoSnapshotSet(wstring bcdFileName)
{
    FunctionTracer ft(DBG_INFO);
    ...
    if (m_dwContext & VSS_VOLSNAP_ATTR_DELAYED_POSTSNAPSHOT)
    {
        ft.WriteLine(L"Fast DoSnapshotSet finished. ");
        return;
    }

    // Check selected writer status
	CheckSelectedWriterStatus(bcdFileName);

    ft.WriteLine(L"Shadow copy set successfully created.");
}


// Ending the backup (calling BackupComplete)
void VssClient::BackupComplete(bool succeeded, wstring bcdFileName)
{
    FunctionTracer ft(DBG_INFO);
...
void VssClient::SaveBackupComponentsDocument(wstring fileName)
{
	...
	wstring strXML;
    WriteFile(fileName, BSTR2WStringRef(strXML, bstrXML));
}
...
void VssClient::GenerateSetvarScript(wstring stringFileName)
{
    ...
    ofile << L"@echo." << endl;
    ofile << L"@echo [This script is generated by VSHADOW.EXE for the shadow set " << Guid2WString(m_latestSnapshotSetID).c_str() << L"]" << endl;
    ofile << L"@echo." << endl;

    wstring snapshotSetID = Guid2WString(m_latestSnapshotSetID);
    ofile << L"SET SHADOW_SET_ID=" << snapshotSetID.c_str() << L"" << endl;
    ...
    for (unsigned i = 0; i < m_latestSnapshotIdList.size(); i++)
    {
        ...
        ofile << L"SET SHADOW_ID_" << i+1 << L"=" << snapshotID.c_str() << L"" << endl;
        ...
        if ((m_dwContext & VSS_VOLSNAP_ATTR_TRANSPORTABLE) == 0)
        {
            ...
            ofile << L"SET SHADOW_DEVICE_" << i+1 << L"=" << Snap.m_pwszSnapshotDeviceObject << L"" << endl;
        }
    }
...
void VssClient::ImportSnapshotSet()
{
    ...
    ft.WriteLine(L"Shadow copy set successfully imported.");
}
