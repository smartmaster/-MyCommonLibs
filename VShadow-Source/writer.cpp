...
void VssClient::GatherWriterMetadataToScreen()
{
    ...
    for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
    {
		...
		MY_WPRINTF(L"--[Writer %u]--\n%s\r\n", iWriter, (LPCWSTR)(bstrXML));
		D_INFO(0, L"--[Writer %u]--\n%s", iWriter, (LPCWSTR)(bstrXML));
    }

	MY_WPRINTF(L"--[end of data]--\r\n");
	D_INFO(0, L"--[end of data]--");
}
...
void VssClient::InitializeWriterMetadata()
{
    ...
    for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
    {
        ...
        //m_writerList.push_back(writer); //<sonmi01>2014-10-10 ###???
    }
}
...
void VssClient::InitializeWriterComponentsForRestore()
{
    ...
    for (unsigned iWriter = 0; iWriter < cWriters; iWriter++)
    {
        ...
        // Otherwise, ignore it!
        bool bFound = false;
		size_t CurrentSize = 0; //<sonmi01>2014-10-21 ###???
        ...
        for (unsigned i = 0; i < m_writerList.size(); i++)
        {
			...
			VssWriter & writer = m_writerComponentsForRestore[CurrentSize];
			++ CurrentSize;
            writer = m_writerList[i];
		...
		m_writerComponentsForRestore.resize(CurrentSize);
...
void VssClient::PreRestore()
{
    ...
    ft.WriteLine(L"Sending the PreRestore event ... ");
...
void VssClient::PostRestore()
{
    ...
    ft.WriteLine(L"Sending the PostRestore event ... ");
...
void VssWriter::Initialize(IVssExamineWriterMetadata * pMetadata)
{
    ...
    for(unsigned i = 0; i < cExcludeFiles; i++)
    {
		...
		VssFileDescriptor & excludedFile = excludedFiles[i];
        excludedFile.Initialize(pFileDesc, VSS_FDT_EXCLUDE_FILES);
        //excludedFiles.push_back(excludedFile); //<sonmi01>2014-10-10 ###???
    }
    ...
    for(unsigned iComponent = 0; iComponent < cComponents; iComponent++)
    {
		...
		VssComponent & component = components[iComponent];
        component.Initialize(name, pComponent);
        //components.push_back(component); //<sonmi01>2014-10-10 ###???
    }
...
void VssComponent::Initialize(wstring writerNameParam, IVssWMComponent * pComponent)
{
    ...
    for(unsigned i = 0; i < pInfo->cFileCount; i++)
    {
		...
		VssFileDescriptor & desc = descriptors[descIndex]; ++ descIndex;
        desc.Initialize(pFileDesc, VSS_FDT_FILELIST);
        //descriptors.push_back(desc); //<sonmi01>2014-10-10 ###???
    }
    ...
    for(unsigned i = 0; i < pInfo->cDependencies; i++)
    {
		...
		VssDependency & dependency = dependencies[i];
        dependency.Initialize(pDependency);
        //dependencies.push_back(dependency); //<sonmi01>2014-10-10 ###???
    }
#endif
