...
#define CURRENT_MODULE_FORMAT					TEXT(" <!--%s-->")
...
{
	CTlsData * pTlsData = CTlsDataManager::GetData(); 
	if (NULL == pTlsData)
	{
		CTlsDataManager::TestAndAllocateData(NULL);
		pTlsData = CTlsDataManager::GetData();
	}
	return pTlsData;
}
...
private:
	...
	IN BOOL m_bDeleteOnClose; /***INCOMMING***/
...
public:
	//////////////////////////////////////////////////////////////////////////
	CDTraceLogger(INT nDebugLevel, BOOL bInXml, LPCTSTR strPrefix, LPCTSTR strLogFolder, BOOL DeleteOnClose);
	~CDTraceLogger();
...
{
public:
	static CDTraceLogger * CreateInstance(INT nDebugLevel, BOOL bInXml, LPCTSTR strPrefix, LPCTSTR strLogFolder, BOOL bDeleteOnClose);
...
CDTraceLogger * CSIDTraceLogger::CreateInstance(INT nDebugLevel, BOOL bInXml, LPCTSTR strPrefix, LPCTSTR strLogFolder, BOOL bDeleteOnClose)
{
	if (!s_pDTraceLogger)
	{
		...
		if (!s_pDTraceLogger)
		{
			s_pDTraceLogger.Attach(new CDTraceLogger(nDebugLevel, bInXml, strPrefix, strLogFolder, bDeleteOnClose));
...
VOID CDTraceLogger::DestroyMemeber()
{
	...
	if (bDeleteEmptyLogFile || m_bDeleteOnClose)
	{
...
//////////////////////////////////////////////////////////////////////////
CDTraceLogger::CDTraceLogger(INT nDebugLevel, BOOL bInXml, LPCTSTR strPrefix, LPCTSTR strLogFolder, BOOL bDeleteOnClose) :
...
{
	static CONST ULONG MAX_FILE_SIZE = 10 * 1024 * 1024 - 1024;
...
//////////////////////////////////////////////////////////////////////////
CDTraceStart::CDTraceStart(INT nDebuglevel, LPCTSTR szEnding0, LPCTSTR	szCloseEnding0, 
						   LPCTSTR szCurrentModule0,
						   LPCTSTR szFunction0, LPCTSTR szFile0, INT Line0
						   ) :
m_szEnding(szEnding0),
m_szCloseEnding(szCloseEnding0),
m_szCurrentModule(szCurrentModule0),
	...
	do 
	{
		...
		if (SILOGGER->IsInXml())
		{
			pTlsData->m_strMessage.Format(
				DETAIL_FORMAT1 TEXT("<%s>\t") DETAIL_FORMAT2 CURRENT_MODULE_FORMAT TEXT("%s"),
		...
		{
			pTlsData->m_strMessage.Format(
				DETAIL_FORMAT1_NOTAG TEXT("Enter %s\t") DETAIL_FORMAT2_NOTAG CURRENT_MODULE_FORMAT TEXT("%s"),
				DETAIL_TEXT1(SystemTime, pTlsData->m_dwProcessID, pTlsData->m_dwThreadID), 
				m_szFunction,
				DETAIL_TEXT2(m_szFunction, m_szFile, m_line),
				m_szCurrentModule,
		...
		OUTPUT_DEBUG_STRING( pTlsData->m_strMessage.GetString() );
...
CDTraceStart::~CDTraceStart()
{
	do 
	{
		...
		{
			pTlsData->m_strMessage.Format(
				DETAIL_FORMAT1_NOTAG TEXT("Leave %s\t") DETAIL_FORMAT2_NOTAG CURRENT_MODULE_FORMAT TEXT("%s"),
				DETAIL_TEXT1(SystemTime, pTlsData->m_dwProcessID, pTlsData->m_dwThreadID), 
				m_szFunction,
				DETAIL_TEXT2(m_szFunction, m_szFile, m_line),
				m_szCurrentModule,
...
//////////////////////////////////////////////////////////////////////////
VOID WINAPI DTraceSetDebugSettings(INT nDebugLevel, BOOL bInXml, LPCTSTR strPrefix, LPCTSTR strLogFolder, BOOL bDeleteOnClose)
{
	CSIDTraceLogger::CreateInstance(nDebugLevel, bInXml, strPrefix, strLogFolder, bDeleteOnClose);
}


//////////////////////////////////////////////////////////////////////////
VOID WINAPIV DTraceRaw(INT nDebugLevel, LPCTSTR szEnding, 
					   LPCTSTR szFormat, ...
						)
{
	do 
						...
						)
{
	do 
	{
		...
		pTlsData->m_strMessage += pTlsData->m_strHelper;
		pTlsData->m_strMessage.AppendFormat(CURRENT_MODULE_FORMAT, szCurrentModule);
		pTlsData->m_strMessage += szEnding;
						  ...
						  ) 
{
	do 
	{
		...
		DWORD dwFormatFlags =	/*FORMAT_MESSAGE_ALLOCATE_BUFFER |*/ //2018-10-8 //!!@@##
		...
		if( INT(dwErrorCode) >= 0)
		{
			...
			{
				pTlsData->m_strMessage.Format(
					DETAIL_FORMAT1 TAG_ERROR_START TEXT("[%s][%d][%s][%s]") TAG_ERROR_END DETAIL_FORMAT2 CURRENT_MODULE_FORMAT TEXT("%s"), 
					DETAIL_TEXT1(SystemTime, pTlsData->m_dwProcessID, pTlsData->m_dwThreadID),
			...
			{
				pTlsData->m_strMessage.Format(
					DETAIL_FORMAT1_NOTAG TAG_ERROR_START_NOTAG TEXT("[%s][%d][%s][%s]") TAG_ERROR_END_NOTAG DETAIL_FORMAT2_NOTAG CURRENT_MODULE_FORMAT TEXT("%s"), 
					DETAIL_TEXT1(SystemTime, pTlsData->m_dwProcessID, pTlsData->m_dwThreadID),
					szApi, dwErrorCode, lpMsgBuf, pTlsData->m_strHelper.GetString(),
					DETAIL_TEXT2(szFunction, szFile, line),
					szCurrentModule,
		...
		else 
		{
			...
			{
				pTlsData->m_strMessage.Format(
					DETAIL_FORMAT1 TAG_ERROR_START TEXT("[%s][0x%08x][%s][%s]") TAG_ERROR_END DETAIL_FORMAT2 CURRENT_MODULE_FORMAT TEXT("%s"), 
					DETAIL_TEXT1(SystemTime, pTlsData->m_dwProcessID, pTlsData->m_dwThreadID),
					szApi, dwErrorCode, lpMsgBuf, pTlsData->m_strHelper.GetString(),
					DETAIL_TEXT2(szFunction, szFile, line),
					szCurrentModule,
			...
			{
				pTlsData->m_strMessage.Format(
					DETAIL_FORMAT1_NOTAG TAG_ERROR_START_NOTAG TEXT("[%s][0x%08x][%s][%s]") TAG_ERROR_END_NOTAG DETAIL_FORMAT2_NOTAG CURRENT_MODULE_FORMAT TEXT("%s"), 
					DETAIL_TEXT1(SystemTime, pTlsData->m_dwProcessID, pTlsData->m_dwThreadID),
					szApi, dwErrorCode, lpMsgBuf, pTlsData->m_strHelper.GetString(),
					DETAIL_TEXT2(szFunction, szFile, line),
					szCurrentModule,
		...
		pTlsData->m_ErrorCodeStringQueue.Put(dwErrorCode, pTlsData->m_strMessage.GetString());
...
//<sonmi01>2014-8-22 ###???
LONG WINAPI DTraceAPIErrQueueCount()
{
	CTlsData * pTlsData = GetTlsDataHelper();
	return pTlsData->m_ErrorCodeStringQueue.GetCount();
}

LONG WINAPI DTraceAPIErrQueueCode(LONG index)
{
	CTlsData * pTlsData = GetTlsDataHelper();
	return pTlsData->m_ErrorCodeStringQueue.GetCode(index);
}

LPCTSTR WINAPI DTraceAPIErrQueueString(LONG index)
{
	CTlsData * pTlsData = GetTlsDataHelper();
	return pTlsData->m_ErrorCodeStringQueue.GetString(index);
}
...
VOID WINAPI DTraceSetFirstStatus(DWORD dwStatusLow, LONG nStatusHigh)
{
	CTlsData * pTlsData = GetTlsDataHelper();

	if (0 == pTlsData->m_liFirstStatus.LowPart)
	{
		pTlsData->m_liFirstStatus.LowPart = dwStatusLow;
	}

	if (0 == pTlsData->m_liFirstStatus.HighPart)
	{
		pTlsData->m_liFirstStatus.HighPart = nStatusHigh;
	}
}

VOID WINAPI DTraceResetFirstStatus(DWORD dwStatusLow, LONG nStatusHigh)
{
	CTlsData * pTlsData = GetTlsDataHelper();
...
VOID WINAPI DTraceSetFirstErrorMessage(LPCTSTR szFormat, ...)
{
	CTlsData * pTlsData = GetTlsDataHelper();
	if (pTlsData->m_strFirstErrorMessage.IsEmpty())
	{
		va_list args;
		va_start(args, szFormat);
		pTlsData->m_strFirstErrorMessage.FormatV(szFormat, args);
		va_end(args);
	}
}

VOID WINAPI DTraceResetFirstErrorMessage(LPCTSTR szFormat, ...)
{
	CTlsData * pTlsData = GetTlsDataHelper();
	//if (pTlsData->m_strFirstErrorMessage.IsEmpty())
	{
...
DWORD WINAPI DTraceGetFirstStatus(LONG * pnStatusHigh)
{
	CTlsData * pTlsData = GetTlsDataHelper();
	if (pnStatusHigh)
	{
		*pnStatusHigh = pTlsData->m_liFirstStatus.HighPart;
	}
	return pTlsData->m_liFirstStatus.LowPart;
}
...
VOID WINAPI DTraceGetFirstErrorMessage(CString & strLastErrorMessage)
{
	CTlsData * pTlsData = GetTlsDataHelper();
	strLastErrorMessage = pTlsData->m_strFirstErrorMessage;
}
