...
namespace SmartLib
{
	...
	class MyFormatCodeTest
	{
	public:
		static void Case3(LPCWSTR src, LPCWSTR dest)
		{
			SmartLib::MyFormatCode::API(src, dest);
		}
		...
		static void Case2(LPCWSTR src, LPCWSTR dest)
		{
			...
			{
				while (line.size() && (line.back() == L'\r' || line.back() == L'\n'))
				{
					line.pop_back();
				}
				raw += line;
			...
			std::wstring formated = MyFormatCode::MyFormatCodeProc(raw.c_str());
		...
		static void Case1()
		{
			LPCWSTR strRaw = SML_STRING_ANY_W(if (1) { 1 + 1; });
			//SML_LOG_LINE(L"raw code is:");
			//SML_LOG_LINE(L"%s", strRaw);
			//SML_LOG_LINE(L"\r\n\r\n");


			std::wstring formated = MyFormatCode::MyFormatCodeProc(strRaw);
			//SML_LOG_LINE(L"formated code is:");
			//SML_LOG_LINE(L"%s", formated.c_str());
		}
		...
		static void Case0()
		{
			LPCWSTR strRaw = SML_STRING_ANY_W(static wstring MyFormatCodeProc(LPCTSTR strRaw)
			{
