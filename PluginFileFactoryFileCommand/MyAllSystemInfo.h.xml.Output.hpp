...
VOID CMyAllSystemInfo::ToElement(LPCTSTR RootName, XmlElement & xmlElement) CONST
{
	xmlElement.m_Name = RootName;

	AllocateOneMore(xmlElement.m_Children);
	System_Info.ToElement(FIELD_NAME(System_Info), Last(xmlElement.m_Children));

	AllocateOneMore(xmlElement.m_Children);
	MSFT_DisksPartitionsVolumes.ToElement(FIELD_NAME(MSFT_DisksPartitionsVolumes), Last(xmlElement.m_Children));

	AllocateOneMore(xmlElement.m_Children);
	AllVolumeDiskInfoXml.ToElement(FIELD_NAME(AllVolumeDiskInfoXml), Last(xmlElement.m_Children));
...
////////////////////////////RRRRRR BEGIN//////////////////////////////////////////////
VOID CMyAllSystemInfo::FromElement(LPCTSTR RootName, CONST XmlElement & xmlElement)
{
	...
	for (size_t ik9856i = 0; ik9856i < xmlElement.m_Children.size(); ++ ik9856i )
	{
		...
		else if (0 == Child_jfs654.m_Name.CompareNoCase(FIELD_NAME(System_Info)))
		{
			System_Info.FromElement(FIELD_NAME(System_Info), Child_jfs654);
		}
		else if (0 == Child_jfs654.m_Name.CompareNoCase(FIELD_NAME(MSFT_DisksPartitionsVolumes)))
		{
			MSFT_DisksPartitionsVolumes.FromElement(FIELD_NAME(MSFT_DisksPartitionsVolumes), Child_jfs654);
		}
		else if (0 == Child_jfs654.m_Name.CompareNoCase(FIELD_NAME(AllVolumeDiskInfoXml)))
		{
			AllVolumeDiskInfoXml.FromElement(FIELD_NAME(AllVolumeDiskInfoXml), Child_jfs654);
		}
	}
}
VOID CMyAllSystemInfo::ToString( LPCTSTR RootName, CString & strXml ) CONST
{
...
HRESULT CMyAllSystemInfo::FromString( LPCTSTR RootName, CONST CString & strXml )
{
	XmlElement xmlElement;
	INT Start = 0;
	HRESULT hr = S_OK;
...
HRESULT CMyAllSystemInfo::ToStream( LPCTSTR RootName, IFileDevice * pIFileDevice ) CONST
{
	HRESULT hr = S_OK;
	BEGIN_BLOCK(0);
	CString strXml;
...
HRESULT CMyAllSystemInfo::FromStream( LPCTSTR RootName, IFileDevice * pIFileDevice )
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;
	BEGIN_BLOCK(0);
...
HRESULT CMyAllSystemInfo::ToFile( LPCTSTR RootName, LPCTSTR pFileName ) CONST
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;
	BEGIN_BLOCK(0);
...
HRESULT CMyAllSystemInfo::FromFile( LPCTSTR RootName, LPCTSTR pFileName )
{
	HRESULT hr = S_OK;
	DWORD LastError = 0;
	BEGIN_BLOCK(0);
